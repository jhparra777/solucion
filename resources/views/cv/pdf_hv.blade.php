                   ...
                   <td colspan="2" width="95%">
                     <p>
                      ...
                      {{ ucwords(mb_strtolower($datos_basicos->nombres)) }} se identifica con la {{mb_strtolower($datos_basicos->dec_tipo_doc)}} número {{ number_format($datos_basicos->numero_id) }} de la ciudad de  {{ \App\Models\Ciudad::GetCiudad($datos_basicos->pais_id, $datos_basicos->departamento_expedicion_id, $datos_basicos->ciudad_expedicion_id) }},
                        ...
                        tiene una aspiración salarial de {{strtolower($datos_basicos->aspiracion_salarial)}}. Reside actualmente en la ciudad de {{ \App\Models\Ciudad::GetCiudad($datos_basicos->pais_residencia, $datos_basicos->departamento_residencia, $datos_basicos->ciudad_residencia) }}
                                ...
                                <p> 
                                    @if($experienciaMayorDuracion)
                                      @if($experienciaMayorDuracion->empleo_actual == 1)
                                       ...
                                       Su experiencia más extensa fue en la empresa {{ ucwords(mb_strtolower($experienciaMayorDuracion->nombre_empresa)) }}, donde se desempeña como {{ mb_strtolower($experienciaMayorDuracion->cargo_especifico) }} @if(\App\Models\Experiencias::añosMeses($experienciaMayorDuracion->fecha_inicio, $experienciaMayorDuracion->fecha_final) != 0) por un periodo de {{ \App\Models\Experiencias::añosMeses($experienciaMayorDuracion->fecha_inicio, $experienciaMayorDuracion->fecha_final) }} @endif. Su experiencia laboral más reciente es en {{ ucwords(mb_strtolower($experienciaActual->nombre_empresa)) }}, desempeñándose como {{ mb_strtolower($experienciaActual->cargo_especifico) }} devengando un salario de {{ mb_strtolower($experienciaActual->salario) }}. Actualmente se encuentra laborando.
                                      
                                      @else
                                      
                                       Su experiencia más extensa fue en la empresa {{ ucwords(mb_strtolower($experienciaMayorDuracion->nombre_empresa)) }}, donde se desempeñó como {{ mb_strtolower($experienciaMayorDuracion->cargo_especifico) }} por un periodo de {{ \App\Models\Experiencias::añosMeses($experienciaMayorDuracion->fecha_inicio, $experienciaMayorDuracion->fecha_final) }}. Su experiencia laboral más reciente fue en {{ ucwords(mb_strtolower($experienciaActual->nombre_empresa)) }}, desempeñándose como {{ mb_strtolower($experienciaActual->cargo_especifico) }} devengando un salario de {{ mb_strtolower($experienciaActual->salario_devengado) }}.
                                      @endif
                                    @endif
                                </p>
    ...
    @if($estudios->count() >= 1)
        ...
        @foreach($estudios as $estudio)
            ...
            <p class="parrafo">
              @if ($estudio->estudio_actual == 1)
                  {{ucwords(mb_strtolower($datos_basicos->nombres))}} está realizando sus estudios en {{ ucwords(mb_strtolower($estudio->institucion)) }} de la ciudad de {{ \App\Models\Ciudad::GetCiudad($estudio->pais_estudio, $estudio->departamento_estudio, $estudio->ciudad_estudio) }}, @if($estudio->semestres_cursados) cursando {{(($estudio->semestres_cursados != 1 )?$estudio->semestres_cursados.' periodos':$estudio->semestres_cursados.' periodo')}} hasta el momento,@endif el título por obtener es {{ ucwords(mb_strtolower($estudio->titulo_obtenido)) }}.
              @else
                  {{ucwords(mb_strtolower($datos_basicos->nombres))}} realizó sus estudios en {{ ucwords(mb_strtolower($estudio->institucion)) }} de la ciudad de {{ \App\Models\Ciudad::GetCiudad($estudio->pais_estudio, $estudio->departamento_estudio, $estudio->ciudad_estudio) }}, @if($estudio->semestres_cursados) cursó {{(($estudio->semestres_cursados != 1 )?$estudio->semestres_cursados.' periodos':$estudio->semestres_cursados.' periodo')}}, @endif finalizando sus estudios el {{ $estudio->getFechaFinalizo() }}, el título obtenido es {{ ucwords(mb_strtolower($estudio->titulo_obtenido)) }}.
              @endif
            </p>
            @endforeach
        @endif
        ...
        @if($experiencias->count() >= 1)
          ...
          <h2>Experiencias</h2>
            ...
            @foreach($experiencias as $key => $experiencia)
                ...
                <p class="subtitulo">
                 <strong> {{ ucwords(mb_strtolower($experiencia->cargo_especifico)) }} de {{ ucwords(mb_strtolower($experiencia->nombre_empresa)) }} </strong>
                </p>

                @if($experiencia->empleo_actual == 1)

                 <p class="parrafo">
                   {{ ucwords(mb_strtolower($datos_basicos->nombres)) }} se desempeña actualmente como {{ ucwords(mb_strtolower($experiencia->cargo_especifico)) }} en la empresa {{ ucwords(mb_strtolower($experiencia->nombre_empresa)) }}, iniciando actividades el {{ $experiencia->getFechaInicia() }} @if($experiencia->ciudad_id) en la ciudad de {{ \App\Models\Ciudad::GetCiudad($experiencia->pais_id, $experiencia->departamento_id, $experiencia->ciudad_id) }} @endif. El salario percibido es de {{ mb_strtolower($experiencia->salario_devengado) }}, su jefe inmediato {{ ucwords(mb_strtolower($experiencia->nombres_jefe)) }} se desempeña como {{ ucwords(mb_strtolower($experiencia->cargo_jefe)) }} @if($experiencia->movil_jefe) y  su número de contacto es {{ $experiencia->movil_jefe }} @endif.
                 </p>

                    <p class="parrafo">
                     @if($experiencia->funciones_logros)
                      Sus principales funciones son {{$experiencia->funciones_logros}}.
                     @endif
                    </p>
                  ...
                  @else
                   ...
                   <p class="parrafo">
                    {{ ucwords(mb_strtolower($datos_basicos->nombres)) }} se desempeñó como {{ ucwords(mb_strtolower($experiencia->cargo_especifico)) }} en la empresa {{ ucwords(mb_strtolower($experiencia->nombre_empresa)) }}, iniciando actividades el {{ $experiencia->getFechaInicia() }} y finalizando el contrato el {{ $experiencia->getFechaFinal() }}, el cual obedeció a {{ ucwords(mb_strtolower($experiencia->desc_motivo))}} @if($experiencia->ciudad_id) en la ciudad de {{ \App\Models\Ciudad::GetCiudad($experiencia->pais_id, $experiencia->departamento_id, $experiencia->ciudad_id) }} @endif. El salario percibido fue de {{ mb_strtolower($experiencia->salario_devengado) }}, su jefe inmediato {{ ucwords(mb_strtolower($experiencia->nombres_jefe)) }} se desempeñó como {{ ucwords(mb_strtolower($experiencia->cargo_jefe)) }} @if($experiencia->movil_jefe) y  su número de contacto es {{ $experiencia->movil_jefe }} @endif.
                   </p>
        ...
        @if($familiares->count() >= 1)
            ...
            @foreach($familiares as $key => $familiar)
                ...
                <p class="parrafo">
                 {{ ucwords(mb_strtolower($familiar->nombres_familiar)) }}, @if($familiar->fecha_nacimiento && $familiar->fecha_nacimiento != "0000-00-00") actualmente tiene {{$familiar->getEdad() }}, @endif el nivel de escolaridad es {{ mb_strtolower($familiar->escolaridad) }}, su profesión es {{ ucwords(mb_strtolower($familiar->profesion_id)) }} @if($familiar->codigo_ciudad_nacimiento) y nació en {{ \App\Models\Ciudad::GetCiudad($familiar->codigo_pais_nacimiento, $familiar->codigo_departamento_nacimiento, $familiar->codigo_ciudad_nacimiento) }} @endif.
                </p>
            @endforeach
        @endif
