...
<head>
	...
	<title>Preview Informe de prueba competencias</title>
...
<body>
	...
	<main class="main">		
		...
		<h3 class="text-center color-sec">Preview Informe de Competencias</h3>
		...
		<section>
			<div class="mt-2 mb-3">
				<table class="m-auto" width="70%">
					...
					<tr>
						<th class="text-uppercase color text-left">
							Fecha realización de la prueba:
						</th>
						<td>
		...
		<section>
			<div class="text-justify color-sec">
				<p>
					Bienvenido, en {{ ucfirst($sitio_informacion->nombre) }} hemos preparado un informe personalizado referente a la evaluación psicotécnica que has realizado.
				</p>

				<p>
					Nuestra prueba psicotécnica Personal Skills evalúa una serie de competencias predefinidas. A continuación vas a encontrar el ajuste al perfil general y el ajuste por cada una de las competencias evaluadas, junto con la descripción y sus perspectivas comportamentales.
				</p>
			</div>
		...
		<section style="height: 280px;" style="margin-top: -5rem;">
			<div class="color-sec" style="float: left; width: 50%; display : inline-block;">
				<div class="text-center mt-3 mb-4">
					...
					<img src="https://ui-avatars.com/api/name={{ round($ajuste_perfil_global) }}&size=80&background=6F3795&color=fff&font-size=0.54&length=3&rounded=true&bold=true" alt="">
				</div>
				...
				<div class="text-center" style="margin-top: -3.5rem;">
					...
					<h1 style="margin-top: -2rem;" class="text-center">
						{{ round($factor_global_desfase) }}%

						@if($factor_global_desfase < 0)
			...
			<div class="text-center mt-2" style="float: right; width: 50%;">
				@if($ajuste_perfil_global < 25)
                    <img src="{{ $url."competencia-barra-circular-01.png" }}" width="380">

                @elseif($ajuste_perfil_global >= 25 && $ajuste_perfil_global <= 50)
                    <img src="{{ $url."competencia-barra-circular-00.png" }}" width="380">

                @elseif($ajuste_perfil_global >= 50 && $ajuste_perfil_global <= 75)

                    @if($ajuste_perfil_global > 50 && $ajuste_perfil_global <= 55)
                        <img src="{{ $url."competencia-barra-circular-02.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 55 && $ajuste_perfil_global <= 58)
                        <img src="{{ $url."competencia-barra-circular-03.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 58 && $ajuste_perfil_global <= 64)
                        <img src="{{ $url."competencia-barra-circular-04.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 64 && $ajuste_perfil_global <= 68)
                        <img src="{{ $url."competencia-barra-circular-05.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 68 && $ajuste_perfil_global <= 72)
                        <img src="{{ $url."competencia-barra-circular-06.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 72 && $ajuste_perfil_global <= 75)
                        <img src="{{ $url."competencia-barra-circular-07.png" }}" width="380">
                    @endif

                @elseif($ajuste_perfil_global >= 75 && $ajuste_perfil_global <= 100)

                    @if($ajuste_perfil_global > 75 && $ajuste_perfil_global <= 78)
                        <img src="{{ $url."competencia-barra-circular-08.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 78 && $ajuste_perfil_global <= 80)
                        <img src="{{ $url."competencia-barra-circular-09.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 80 && $ajuste_perfil_global <= 84)
                        <img src="{{ $url."competencia-barra-circular-10.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 84 && $ajuste_perfil_global <= 94)
                        <img src="{{ $url."competencia-barra-circular-11.png" }}" width="380">

                    @elseif($ajuste_perfil_global > 94 && $ajuste_perfil_global <= 100)
					...
					@foreach($pskills_fotos as $key => $foto)
						@if(!triRoute::validateOR('local'))
							<img 
								...
								src="{{ asset("recursos_prueba_ps_sola/prueba_ps_sola_"."$candidato_prueba->user_id"."_"."$candidato_prueba->id/$foto->descripcion") }}" 
