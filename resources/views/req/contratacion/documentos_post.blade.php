...
<div class="row">
    <div class="col-sm-6">
        <h2>Documentos post contratacion</h2>
                                        ...
                                        @foreach($tipo_documento as $tipo)
                                            <tr>
                                                ...
                                                @if( count($tipo->documentos) > 0)
                                                    ...
                                                    <td> {{ (isset($tipo->usuario_gestiono) && isset($tipo->fecha_carga) )?date("d-m-Y",strtotime($tipo->fecha_carga)):'' }} </td>
                                                    <td>
                                                        <i class="fa fa-check" aria-hidden="true" style="color:green; margin-right: 4px;"></i>

                                                        @foreach($tipo->documentos as $doc)
                                                            <a href='{{ asset("recursos_documentos_verificados/$doc->nombre") }}' target="_blank"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>

                                                            <a href='{{ route('admin.descargar_recurso', ['recursos_documentos_verificados', $doc->nombre]) }}' target="_blank" title="Descargar archivo" style="margin-right: 6px;">
