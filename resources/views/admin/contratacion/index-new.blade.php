                                        ...
                                        @if($sitio->precontrata == 1)
                                            ...
                                            @if ($candidato->nombre_proceso == 'PRE_CONTRATAR' || $ultimo_proceso->proceso == 'CONTRATO_ANULADO' || $ultimo_proceso->proceso == 'CANCELA_CONTRATACION')
                                                <button
                                                    ...
                                                    @if($candidato->estado_candidato==config('conf_aplicacion.C_EN_PROCESO_CONTRATACION') || $candidato->estado_candidato==config('conf_aplicacion.C_CONTRATADO') || !$candidato->sst_check || !$candidato->validacion_documental_check || $candidato->tiene_estudio == null )
                                                        disabled
                                                        @if ( $candidato->tiene_estudio == null )
                                                        title="El candidato debe tener al menos un registro de estudios realizados"
                                                        @endif
                                                    @endif
                                                >
                                                    CONTRATAR
