...
@section('contenedor')
    ...
    @endif 
        @if ( count($tiene_asociado_proceso) > 0)
    ...
    <div class="row">
        ...
        <h3 class="titulo1">Entrevistas Realizadas</h3>
            ...
            @foreach($entrevistas_semi as $entrevista)

                <div class="container_referencia">
                    <div class="referencia">
                        <table class="table table-bordered tbl_info" style="margin-bottom: 0px">

                            <button class="btn btn-info ver_detalle_semi" data-id="{{$entrevista->id}}">Ver detalle</button>
                    ...
                    <div class="requerimientos">
                        ...
                        <div class="btn_procesos">
                            ...
                            <label class="switchBtn">
                                 {!! Form::checkbox("definitiva",0,($entrevista->getRequerimientosActivos($candidato->requerimiento_id)!= null)? 1:null,["class"=>"usar","id"=>"switch","data-entrevista"=>$entrevista->id,"data-req"=>$candidato->requerimiento_id,"data-tipo"=>"semi"]) !!}
                                <div class="slide"></div>
                            </label>
                        </div>
   ...
   content: "SI"  !important;
    ...
    <script>
        ...
        if(!$(this).prop("checked")){
            ...
            if(tipo=="semi"){
                var url="{{ route('admin.registra_entrevista_entidad_semi') }}";
            }
            else{
                var url="{{ route('admin.registra_entrevista_entidad') }}";
            }
            //var btn = $(this);
            ...
            var tipo=$(this).data("tipo");
            if(tipo=="semi"){
                var url="{{ route('admin.registra_entrevista_entidad2_semi') }}";
            }
            else{
                var url="{{ route('admin.registra_entrevista_entidad2') }}";
            }
            //var btn = $(this);
            alert(url);
            $.ajax({
            ...
            $(document).on("click", "#guardar_entrevista_semi", function (e) {
                e.preventDefault();
