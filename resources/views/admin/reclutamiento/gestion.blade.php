                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <ul class="dropdown-menu" role="menu" id="botones_de_informacion">
                                                        ...
                                                        @endif
                                                            ...
                                                            <a type="button" style="width: 100%;text-decoration:none;color:white;" class="btn btn-sm btn-info" {!! ((funcionesglobales::validabotonprocesos($candidato_req->req_candidato_id, ["ENVIO_ENTREVISTA"]))? :"disabled = 'disabled'") !!} target="_blank" href="{{route('admin.informe_entrevista_semi_pdf',['req_can'=>$candidato_req->req_candidato_id])}}"> ENTREVISTA SEMI PDF</a>
...
<script>
    ...
    function actualizar_trazabilidad_individual(tipo,req_cand, id_proceso){
        switch(tipo) {
            ...
            case "ENV_EST_SEG":
                ...
                break;
            case "ENV_SST":
                var url = "{{route('gestionar_evaluacion_sst', ':id') }}";
                break;
            default:
                var url = "";
        } 
    ...
    $(function() {
       ...
       $('#table_with_users').delegate( '.btn_observaciones', 'click', function(){
        ...
        //Evaluacion SST
        $(".btn_sst_masivo").on("click", function() {
       
            if ($("input[name='req_candidato[]']").serialize() != "") {
                $.ajax({
                    type: "POST",
                    data: $("input[name='req_candidato[]']").serialize(),
                    url: "{{ route('admin.enviar_sst_masivo') }}",
                    success: function(response) {
                        $("#modal_peq").find(".modal-content").html(response);
                        $("#modal_peq").modal("show");
                    }
        ...
        $(document).on("click", "#confirmar_sst_masivo", function() {
            if($('#fr_sst_masi').smkValidate()){
                $.ajax({
                    type: "POST",
                    data: $("#fr_sst_masi").serialize(),
                    url: "{{ route('admin.confirmar_sst_masivo') }}",
                    beforeSend: function(){
                        $("#modal_peq").modal("hide");
                        //imagen de carga
                        $.blockUI({
                    ...
                    success: function(response) {
                        ...
                        if(response.success) {
                            ...
                            var mensaje = 'Se han enviado a los candidatos satisfactoriamente.';
                            if (response.candidatos_no_enviados.length > 0) {
                                mensaje += "<br>Pero " + response.candidatos_no_enviados.length + " candidatos ya se encuentran en proceso de realizar Evaluación SST y no se han enviado:<br>";
                                response.candidatos_no_enviados.forEach(function(cand, index) {
                                    mensaje += '<b>' + cand + '</b>';
                                    if (response.candidatos_no_enviados.length - 1 > index) {
                                        mensaje += ' - ';
        ...
        //rodamiento esto rodamiento
        $(document).on("click","#rodamiento", function(e) {
            var req = {{$requermiento->id}};

            $.ajax({
              type: "POST",
              data:{req:req},
              url: "{{route('admin.rodamiento_view')}}",
              success: function(response) {
                $("#modal_peq").find(".modal-content").html(response);
                $("#modal_peq").modal("show");
                $("#req").val(req);
        ...
        //confirmar rodamiento

        $(document).on("click", "#confirmar_rodamiento", function() {
            
           $.ajax({
              type: "POST",
              data: $("#fr_rodamiento").serialize(),
              url: "{{route('admin.confirmar_rodamiento')}}",
              success: function(response){
               if(response.success) {
                mensaje_success("Se guardado rodamiento.");
                 location.reload();
               }else{ 
                //$("#modal_peq").find(".modal-content").html(response.view);
                $("#modal_peq").modal("show");
        ...
        @if(route('home')== "https://gpc.t3rs.co" || route("home") == "http://localhost:8000")
