<script>
    ...
    $(function() {
        ...
        */
        ...
        /**
         * Confirmar reusar prueba
        */

        $(document).on("click", "#confirmar_pruebas_reusar", (e) => {
            if ($('#form_procesos_reusables').smkValidate()) {
                e.target.setAttribute('disabled', 'disabled')

                $.ajax({
                    type: "POST",
                    data: $("#form_procesos_reusables").serialize(),
                    url: "{{ route('admin.gestion.reusar_pruebas') }}",
                    success: function(response) {
                        if(response.success) {
