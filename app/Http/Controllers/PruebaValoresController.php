...
class PruebaValoresController extends Controller
{
    ...
    //Generar informe preliminar a reusar de la prueba de valores con los resultados de otro requerimiento o de la prueba general
    public function preliminar_reusable_prueba_valores($tipo, $respuesta_id, $req_id)
    {
        ...
        $proceso = new RegistroProceso();
        $proceso->candidato_id = $candidato_valores->user_id;
        $proceso->proceso = 'ENVIO_PRUEBA_ETHICAL_VALUES';
        $proceso->requerimiento_id = $req_id;
        $proceso->usuario_envio = $this->user->id;
        $proceso->created_at = Date('Y-m-d');
        ...
        $configuracion = PruebaValoresConfigRequerimiento::where('req_id', $req_id)->orderBy('id', 'desc')->first();
        ...
        $requerimiento_detalle = Requerimiento::where('requerimientos.id', $req_id)->first();
