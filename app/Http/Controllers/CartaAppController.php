...
class CartaAppController extends Controller
{
    public function adelanto_nomina()
    {
        ...
        $solicitud = CartappSolicitudUser::where('user_id', $this->user->id)
        ...
        if (!is_null($solicitud)) {
            if ($solicitud->created_at->formatLocalized('%B') == $mes_actual) {
                if ($periodo_solicitud == 'primer_periodo') {
                    if (($solicitud->created_at->day >= $periodo_evaluar->primer_periodo_dia_inferior) && ($solicitud->created_at->day <= $periodo_evaluar->primer_periodo_dia_superior)) {
                        //Si ya hizo una solicitud en el presente periodo no se permite hacer otra solicitud
                        $candidato = null;
                        $mensaje = 'Ya has solicitado un adelanto de nómina en este periodo. Podrás volver a pedir tu adelanto a partir del <b>día ' . $periodo_evaluar->segundo_periodo_dia_inferior . '</b> de <b>' . $mes_actual . '</b>.';
                        return view("cv.adelanto_nomina", compact(
                            "candidato",
                            "menu",
                            "mensaje"
                    ...
                    if (($solicitud->created_at->day >= $periodo_evaluar->segundo_periodo_dia_inferior) && ($solicitud->created_at->day <= $periodo_evaluar->segundo_periodo_dia_superior)) {
                        //Si ya hizo una solicitud en el presente periodo no se permite hacer otra solicitud
                        $candidato = null;
                        $fecha_hoy_carbon->addMonth();
                        $mensaje = 'Ya has solicitado un adelanto de nómina en este periodo. Podrás volver a pedir tu adelanto a partir del <b>día ' . $periodo_evaluar->primer_periodo_dia_inferior . '</b> de <b>' . $fecha_hoy_carbon->formatLocalized('%B') . '</b>.';
                        return view("cv.adelanto_nomina", compact(
                            "candidato",
                            "menu",
                            "mensaje"
    ...
    public function verificar_codigo_adelanto_nomina_async(Request $request)
    {
        ...
        if ($candidato->codigo_adelanto_nomina == $request->code) {
            ...
            return response()->json(["success" => true, "view" => view("cv.includes.adelanto_nomina.condiciones_adelanto_nomina", compact("candidato", "sitio", "solicitud", "correo_responsable"))->render()]);
        }
