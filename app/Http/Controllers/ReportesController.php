              ...
              $join2->on("ciudad.cod_ciudad", "=", "datos_basicos.ciudad_residencia")->on("ciudad.cod_departamento", "=", "datos_basicos.departamento_residencia");
                ...
                ->select(
                    ...
                    'datos_basicos.direccion',
                     ...
                     DB::raw('(select ciudad.nombre from ciudad where ciudad.cod_pais = datos_basicos.pais_id and ciudad.cod_departamento = datos_basicos.departamento_expedicion_id and ciudad.cod_ciudad = datos_basicos.ciudad_expedicion_id LIMIT 1) as ciudad_expedicion'),
...
//dd($data);
    ...
    */
    ...
    public function reportesRegistroAceptacion(Request $request)
    {
      //$usuarios = DatosBasicos::where('acepto_politicas_privacidad', 1)->get();
      $headersr  = $this->getHeaderDetalleRegistroAceptacion();
      $data      = $this->getDataDetalleRegistroAceptacion($request);
          
        return view('admin.reportes.reporte_aceptacion_ley')->with([
            'data'      => $data,
            'headersr'   => $headersr
        ]);
    }

    public function reportesRegistroAceptacionExcel(Request $request)
    {
      $headersr  = $this->getHeaderDetalleRegistroAceptacion();
        $data      = $this->getDataDetalleRegistroAceptacion($request);
        $formato = $request->formato;
        ...
        Excel::create('reporte-excel-aceptacion-ley', function ($excel) use ($data, $headersr, $formato) {
            $excel->setTitle('Registro aceptación  - ley 1581');
            $excel->setCreator('$nombre')
                ->setCompany('$nombre');
            $excel->setDescription('Reporte Registro aceptación  - ley 1581');
            $excel->sheet('Registro aceptación  - ley 1581', function ($sheet) use ($data, $headersr, $formato) {
                $sheet->setOrientation("landscape");
                $sheet->loadView('admin.reportes.includes.grilla_detalle_registro_aceptacion', [
                    'data'    => $data,
                    'headersr' => $headersr,
                    'formato' => $formato,
                ]);
  ...
  public function reportesDescargaContratacion(Request $request){
    ...
    */
    ...
    private function getHeaderDetalleRegistroAceptacion()
    {
        $headersr = [
            'Nro. de Identificación',
            'Primer Apellido',
            ...
            'Teléfono móvil',
            'Correo electrónico',
            'Fecha de aceptación de política',
            'Hora de aceptación de política',
            'Ip de donde acepto de política'
            ];
        return $headersr;
    }
    ...
    */
    ...
    private function getDataDetalleRegistroAceptacion($request)
    {
        if(isset($request->numero_id)){
        ...
        if($numero_id != '' || $rango_fecha != ''){
            ...
            $data = DatosBasicos::where('acepto_politicas_privacidad', 1)->cedula($numero_id)->rangoFecha($fecha_inicio, $fecha_final);

        } else if (isset($generar_datos)) {
            session()->flash('mensaje_warning', 'Debe ingresar el Número de cédula o seleccionar un rango de fecha de la aceptación de la política');
        }
    ...
    */
    ...
    private function getHeaderEnviadosExamMedicos()
    {
        $headersr = [
            ...
            'Cargo',
            'Agencia req.',
            'Fecha envío a examen',
            'Fecha de carga de examen',
            'Hora de carga de examen',
            'Fecha realización examen',
            'Resultado',
            'Motivo',
            'Usuario envió'
        ];
        //#Identificación, Nombres y Apellidos, #Req, Cliente, Fecha envío a examen,  Fecha Realización Examen, Resultado, Motivo, Usuario envió
        return $headersr;
    }
    ...
    private function getDataEnviadosExamMedicos($request)
    {
      ...
      $data = "vacio";
      if ($rango_fecha != "" || $numero_id != "") {
        $data = OrdenMedica::join("procesos_candidato_req", "procesos_candidato_req.requerimiento_candidato_id", "=", "orden_medica.req_can_id")
          ->join("datos_basicos", "datos_basicos.user_id", "=", "procesos_candidato_req.candidato_id")
          ->join("requerimientos", "requerimientos.id", "=", "procesos_candidato_req.requerimiento_id")
          ->join("cargos_especificos", "cargos_especificos.id", "=", "requerimientos.cargo_especifico_id")
          ->join('negocio', 'requerimientos.negocio_id', '=', 'negocio.id')
          ->join('clientes', 'negocio.cliente_id', '=', 'clientes.id')
          ->leftjoin('motivo_rechazo_examen_medico', 'motivo_rechazo_examen_medico.id', '=', 'orden_medica.motivo_rechazo')
          ...
          ->where(function($sql) use ($fecha_inicio, $fecha_final, $numero_id, $candidatos_con_examenes)
          {
              if ($fecha_inicio != "" && $fecha_final != "") {
              ...
              if ($candidatos_con_examenes != null && $candidatos_con_examenes != "") {
                  $sql->whereNotNull('orden_medica.resultado');
              }
          })
          ...
          ->select(
              ...
              "datos_basicos.numero_id",
              "cargos_especificos.descripcion as cargo",
              "motivo_rechazo_examen_medico.descripcion as motivo_rechazo",
              "orden_medica.id as orden_id",
              "orden_medica.created_at as fecha_envio",
              ...
              DB::raw("(select users.name from users where procesos_candidato_req.usuario_envio = users.id AND procesos_candidato_req.requerimiento_id = requerimientos.id LIMIT 1) as usuario_envio_name"),
              DB::raw("(SELECT documentos.fecha_realizacion FROM documentos WHERE documentos.user_id = procesos_candidato_req.candidato_id AND documentos.requerimiento = procesos_candidato_req.requerimiento_id AND documentos.descripcion_archivo IN ('EXAMENES MÉDICOS', 'Resultado de examen medico') ORDER BY created_at DESC LIMIT 1) as fecha_realizacion"),
              DB::raw("(SELECT documentos.created_at FROM documentos WHERE documentos.user_id = procesos_candidato_req.candidato_id AND documentos.requerimiento = procesos_candidato_req.requerimiento_id AND documentos.descripcion_archivo IN ('EXAMENES MÉDICOS', 'Resultado de examen medico') ORDER BY created_at DESC LIMIT 1) as fecha_carga"),
              DB::raw("(SELECT documentos.resultado FROM documentos WHERE documentos.user_id = procesos_candidato_req.candidato_id AND documentos.requerimiento = procesos_candidato_req.requerimiento_id AND documentos.descripcion_archivo IN ('EXAMENES MÉDICOS', 'Resultado de examen medico') ORDER BY created_at DESC LIMIT 1) as resultado")
          )
        ->orderBy("procesos_candidato_req.created_at", "desc");
      } else if (isset($generar_datos)) {
        session()->flash('mensaje_warning', 'Debe seleccionar un rango de fecha o agregar nro. de identificación');
      }
      ...
      if($data != "vacio"){
        ...
        /*foreach ($data as &$candidato) {
            ...
            $doc_candidato = Documentos::where('user_id', $candidato->user_id)
              ...
              ->whereIn('descripcion_archivo', ['EXAMENES MÉDICOS', 'Resultado de examen medico'])
    ...
    private function getDataSeguimiento(Request $request){
       ...
       // dd($request->all());
        ...
        if ($fecha_inicio != '' || $fecha_final != '' || $cliente_id != '' || $criterio!= '' || $area1!='' || $num_req!='' || $agencia!='' || $usuario!='' || $empresa_contrata!='') {
            ...
            $data=Requerimiento::join('tipo_proceso','tipo_proceso.id','=','requerimientos.tipo_proceso_id')
            //->join('requerimientos_estados', 'requerimientos.id', '=', 'requerimientos_estados.req_id')
            ->leftjoin('empresa_logos', 'empresa_logos.id', '=', 'requerimientos.empresa_contrata')
            ->join('estados_requerimiento', 'requerimientos.id', '=', 'estados_requerimiento.req_id')
            ->join('estados','estados.id',"=","estados_requerimiento.estado")
