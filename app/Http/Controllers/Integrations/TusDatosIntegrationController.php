...
class TusDatosIntegrationController extends Controller
{
    ...
    public function enviarConsulta(Request $request)
    {
        ...
        if ($consultas_realizadas >= (int) $limite->limite) {
            ...
            $datos_basicos = DatosBasicos::where('user_id', $user_id)->select('tipo_id', 'fecha_expedicion')->first();
        ...
        return response()->json([
            'view' => view('admin.reclutamiento.modal._modal_consultar_tusdatos', compact('req_id', 'user_id', 'datos_basicos', 'tipos_documentos'))->render(),
            'success' => $success,
            'limite' => $limite
        ]);
    }
