...
class CargosEspecificosController extends Controller
{
    ...
    public function editar(Request $request)
    {
        ...
        if($sitio->asistente_contratacion == 1) {
            ...
            $adicionales = DocumentoAdicional::where('active', 1)->whereNotIn('id', $adicionales_cliente->modelKeys())->where('creada', 1)->orWhere('default', 1)->get();
        }
    ...
    public function actualizar(Request $datos)
    {
        ...
        if($datos->has("clausulas") && is_array($datos->get("clausulas"))) {
            ...
            DB::table('cargos_documentos_adicionales')->where('cargo_id', $registro->id)->delete();

            foreach($datos->get("clausulas") as $clausula) {

                $usuario_agencia = new CargoDocumentoAdicional();

                $usuario_agencia->cargo_id = $registro->id;
                $usuario_agencia->adicional_id = $clausula;
    ...
    public function nuevo(Request $data)
    {
        ...
        if($sitio->asistente_contratacion == 1) {
            $tipos_documento = TipoDocumento::where('active', 1)->get();
            $tipos_examenes = ExamenMedico::where('status', 1)->get();
            $adicionales = DocumentoAdicional::where('creada', 1)->orWhere('default', 1)->get();
        }
	...
	public function guardar(Request $data, Requests\CargosEspecificosNuevoRequest $valida)
	{
        ...
        $registro = new ModeloTabla();
        $adicionales_default = DocumentoAdicional::where('default', 1)->get();
        ...
        /*if($data->has("documento") && is_array($data->get("documento"))) {
            ...
            foreach($documento_cargo as $value){
              	if(!in_array($value,$result)){
                	DB::table('cargo_documento')->where('cargo_especifico_id', $registro->id)
                    ->where("tipo_documento_id", $value)
                    ->delete();
              	}
            }
        }*/
        ...
        //Asociar clausulas al cargo
        $sitio = Sitio::first();

        /*if($sitio->asistente_contratacion == 1) {
            $getDocumentos = DocumentoAdicional::where('creada', 0)->get();

            if(count($getDocumentos) > 0) {
                foreach ($getDocumentos as $asignDocumentos) {
                    $clausulas = new CargoDocumentoAdicional();

                    $clausulas->cargo_id = $registro->id;
                    $clausulas->adicional_id = $asignDocumentos->id;
                    $clausulas->save();
                }
            }
        }*/
