...
class ReclutamientoController extends Controller
{
    ...
    public function gestion_req($req_id, Request $data)
    {
        ...
        $requermiento = Requerimiento::leftjoin("motivo_requerimiento", "motivo_requerimiento.id", "=", "requerimientos.motivo_requerimiento_id")
    ...
    */
    ...
    public function RegistroProceso($campos = array(), $estado, $candidato_req)
    {
        ...
        if ($estado == config('conf_aplicacion.C_EVALUACION_DEL_CLIENTE')) {
            ...
            if($requerimiento_obj->tipo_proceso_id==2){
                if($requerimiento_obj->num_vacantes<=5){
                    $cant_enviar=$requerimiento_obj->num_vacantes*3;
                }
                else{
                    $cant_enviar=$requerimiento_obj->num_vacantes*2;
    ...
    public function enviar_examenes_view(Request $data)
    {
        ...
        if($sitio_modulo->usa_ordenes_medicas == 'si') {
            ...
            ->where("examen_medico.status", 1)
            ->select(
                "examen_medico.nombre as nombre",
                "examen_medico.id as id",
                "cargos_examenes.cargo_id as cargo"
            )
            ->orderBy("examen_medico.nombre", "ASC")
            ->groupBy("examen_medico.id")
            ->get();
    ...
    public function confirmar_prueba(Request $data)
    {
        ...
        ->where('requerimiento_cantidato.id', $data->get("candidato_req"))
        ->select(
            'requerimiento_cantidato.id as cand_req_id',
            'requerimiento_cantidato.requerimiento_id as req_id',
        ...
        /**
         * Recorrer las pruebas seleccionadas en el modal
         */
        ...
        for ($i = 0; $i < count($pruebas); $i++) { 
            ...
            if($pruebas[$i] == 1){
                ...
                $pruebasEnviadas[] = 'Prueba Externa';
            }
            ...
            if($pruebas[$i] == 3 || $pruebas[$i] == 4 || $pruebas[$i] == 6){
                if($pruebas[$i] == 3) {
                    ...
                    $pruebasEnviadas[] = 'Prueba Excel Básico';
                } else if($pruebas[$i] == 4) {
                    ...
                    $pruebasEnviadas[] = 'Prueba Excel Intermedio';
                } else if($pruebas[$i] == 6) {
                    ...
                    $respEthicalValuesGral = PruebaValoresRespuestasGeneral::where('user_id', $datos_candidato->user_id)->orderBy('fecha_respuesta', 'DESC')
                    ->first();

                    $respEthicalValues = PruebaValoresRespuestas::where('user_id', $datos_candidato->user_id)
                        ->orderBy('fecha_respuesta', 'DESC')
                    ->first();

                    if (!is_null($respEthicalValuesGral) || !is_null($respEthicalValues)) {
                        $reusar = true;
                        $pruebasReusar[] = 'valores';

                        if (!is_null($respEthicalValuesGral) && !is_null($respEthicalValues)) {
                            $fechaEVGral = Carbon::parse($respEthicalValuesGral->fecha_respuesta);
                            $fechaEVReq = Carbon::parse($respEthicalValues->fecha_respuesta);
                            if ($fechaEVReq >= $fechaEVGral) {
                                $reusarValores[] = array('id' => $respEthicalValues->id, 'fecha' => $respEthicalValues->fecha_respuesta, 'prueba' => 'requerimiento');
                            } else {
                                $reusarValores[] = array('id' => $respEthicalValuesGral->id, 'fecha' => $respEthicalValuesGral->fecha_respuesta, 'prueba' => 'general');
                            }
                        } else if (!is_null($respEthicalValuesGral)) {
                            $reusarValores[] = array('id' => $respEthicalValuesGral->id, 'fecha' => $respEthicalValuesGral->fecha_respuesta, 'prueba' => 'general');
                        } else if (!is_null($respEthicalValues)) {
                            $reusarValores[] = array('id' => $respEthicalValues->id, 'fecha' => $respEthicalValues->fecha_respuesta, 'prueba' => 'requerimiento');
                        }

                        continue;
                    } else {
                        $pruebasEnviadas[] = 'Prueba Ethical Values';
                    }
                }
            ...
            //Competencias
            if($pruebas[$i] == 5) {
                $respCompetenciasSola = PruebaCompetenciaSolaResultado::where('user_id', $datos_candidato->user_id)->orderBy('fecha_realizacion', 'DESC')->first();

                if (!is_null($respCompetenciasSola)) {
                    $reusar = true;
                    $pruebasReusar[] = 'competencias';
                    $reusarCompetencia = ['id' => $respCompetenciasSola->id, 'fecha' => $respCompetenciasSola->fecha_realizacion]; // Asignar el id de la prueba a reusar
                ...
                $pruebasEnviadas[] = 'Prueba Competencias';
            }
            ...
            if ($pruebas[$i] == 7) {
                ...
                */
                ...
                $pruebasEnviadas[] = 'Prueba Digitación';
            ...
            if($pruebas[$i] == 2){
                ...
                $pruebaBrygCandidato = PruebaBrigSolaResultado::where('user_id', $datos_candidato->user_id)
                ->where('estado', 1)
                ...
                if (!empty($pruebaBrygCandidato)) {
                    $reusar = true;
                    $pruebasReusar[] = 'bryg';
                    $reusarBryg = ['id' => $pruebaBrygCandidato->id, 'fecha' => $pruebaBrygCandidato->fecha_realizacion];
        ...
        $view = '';
        if ($reusar) {
            $view = view("admin.reclutamiento.modal._modal_pruebas_reusables", compact("pruebasReusar", "pruebasEnviadas", "reusarValores", "reusarCompetencia", "reusarBryg", "datos_candidato"))->render();
        }

        return response()->json([
            "success" => $reusar ? false : true,
    ...
    //dotacion de candidatos
    public function rodamiento_view(Request $data)
    {
       $req = Requerimiento::find($data->req);
       $dotacion = $req->detalle_dotacion;

      return view("admin.reclutamiento.modal.detalle_rodamiento",compact('dotacion','req'));
    }
    //
    ...
    public function confirmar_dotacion(Request $data)
    {
     ...
     //
    public function confirmar_rodamiento(Request $data)
    {
        $campos = ['detalle_rodamiento' => $data->get("detalle_rodamiento"),
         'req' => $data->get("req")];

         $req = Requerimiento::find($campos["req"]);
         $req->detalle_rodamiento = $campos['detalle_rodamiento'];
         $req->save();
    ...
    //SST masivo
    public function enviar_sst_masivo(Request $data) {
        $datos_basicos =[];
        $req_can_id = [];
        ...
        foreach($data->req_candidato as $key => $req_candi_id) {
            ...
            array_push($datos_basicos, $candidato->numero_id . ' ' . $candidato->nombres . ' ' . $candidato->primer_apellido);
            array_push($req_can_id, $candidato->req_candidato);
        }
        
        $req_can_id_j = json_encode($req_can_id);
         
        return view("admin.reclutamiento.modal.enviar_sst_masivo", compact("datos_basicos", "req_can_id_j"));
    }

    public function guardar_sst_masivo(Request $data)
    {
        ...
        foreach ($requerimientos_cand_id as $key => $id_req_cand) {
            ...
            $proceso_sst = RegistroProceso::where('requerimiento_candidato_id', $id_req_cand)->where('proceso', 'ENVIO_SST')->first();

            if ($proceso_sst != null) {
                array_push($candidatoNoEnviado, "$candidato->numero_id $candidato->nombres");
            } else {
                $campos = [
                ...
                $id_proceso = $this->RegistroProceso($campos, config('conf_aplicacion.C_EN_PROCESO_SELECCION'), $id_req_cand);
                ...
                $mailBody = "
                            Buen dia,
                            <br/><br/>
                            Hola {$nombre_completo}, te informamos que nuestro analista de selección te ha solicitado leer el archivo adjunto y responder la evaluación de seguridad y salud en el trabajo que preparamos para ti! Por favor haz clic en el botón “Realizar Evaluación SST”, ingresa a la plataforma usando tu número de identificación como usuario (sin puntos, comas ni signos) y también como contraseña.
                    ";
                //Arreglo para el botón
                $mailButton = ['buttonText' => 'Realizar Evaluación SST', 'buttonRoute' => route('realizar_evaluacion_sst')];
                ...
                Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use($candidato) {
                        ...
                        $message->subject("Evaluación SST")
                                ->to([$candidato->email],'Soluciones')
                                ->attach(public_path('FOLLETO_EVALUACION.pdf'))
                                ->getHeaders()
                                ->addTextHeader('X-SES-CONFIGURATION-SET', 'watch_emails_t3rs');
                });
    ...
    public function enviar_aprobar_cliente_view_masivo(Request $data)
    {
        ...
        ->join("negocio", "negocio.id", "=", "requerimientos.negocio_id")
        ->join("clientes", "clientes.id", "=", "negocio.cliente_id")
        ->select('requerimiento_cantidato.requerimiento_id', "clientes.id as cliente","requerimientos.tipo_proceso_id")
        ->first();
        ...
        foreach ($data->req_candidato as $key => $req_candi_id) {
            ...
            if($req_id->tipo_proceso_id==2){
                if($candidato->procesos->where('proceso','ENVIO_ENTREVISTA')->first()->apto!=null){
                array_push($datos_basicos, $candidato->nombres.' '.$candidato->primer_apellido);
                array_push($req_can_id, $candidato->req_candidato);
                }
                else{
                    array_push($datos_basicos_fault, $candidato->nombres.' '.$candidato->primer_apellido);
                }
            }
            else{
                array_push($datos_basicos, $candidato->nombres.' '.$candidato->primer_apellido);
        ...
        return view("admin.reclutamiento.modal.enviar_aprobar_cliente_masivo", compact("datos_basicos", "req_can_id_j", "req_id", "usuarios_clientes","datos_basicos_fault"));
    }
    ...
    public function confirmar_aprobar_cliente(Request $data)
    {
        ...
        $mailTitle = "Aprobación de candidatos"; //Titulo o tema del correo
         ...
         //Cuerpo con html y comillas dobles para las variables
        $mailBody = "
                    Hola {$user_envio->name}, te informamos que el analista de selección que está llevando a cabo el proceso en el requerimiento No. {$requerimiento->id} para el cargo {$requerimiento->cargo_especifico} te ha enviado al candidato {$candidato->name} para aprobación. Por favor ingresa por el siguiente botón para realizar la gestión.";
    ...
    public function enviar_contratar2(Request $data)
    {
        ...
        if ($candidato != null && $sitio->asistente_contratacion == 1) {
            if ($candidato->tipo_id == null || $candidato->tipo_id == 0 || $candidato->direccion == null || $candidato->entidad_eps == null || $candidato->entidad_afp == null || $candidato->fecha_expedicion == null || $candidato->fecha_nacimiento == null || $candidato->ciudad_expedicion_id == null || $candidato->genero == null) {
                $candidato->observacion_no_cumple = "El candidato debe completar los datos Tipo de documento, Dirección, Eps, Afp, Fecha de expedición documento, Fecha de nacimiento, Lugar de residencia, Teléfono, Lugar expedición,Género";
                $candi_no_cumplen = $candidato;
                $candidato = null;
            }
        }
    ...
    public function enviar_a_contratar_cliente_req(Request $data)
    {
        ...
        $req = \App\Models\RegistroProceso::where("proceso", "ENVIO_APROBAR_CLIENTE")->where("requerimiento_candidato_id", $data->candidato_req)->first();

        $requerimiento = Requerimiento::find($req->requerimiento_id);
        ...
        if(route("home") == "http://temporizar.t3rs.co" || route("home") == "https://temporizar.t3rs.co" ||
            route("home") == "http://desarrollo.t3rs.co" || route("home") == "https://desarrollo.t3rs.co" ||
            route("home") == "https://pruebassoluciones.t3rsc.co"){
        ...
        if(route("home") == "http://demo.t3rs.co" || route("home") == "https://demo.t3rs.co" ||
            route("home") == "http://localhost:8000" || route("home") == "http://desarrollo.t3rs.co" ||
            route("home") == "https://desarrollo.t3rs.co" || route("home")=="https://pruebaslistos.t3rs.co" ||
            route("home")=="https://listos.t3rs.co" || route("home")=="https://vym.t3rs.co" ||
            route("home") == "https://pruebassoluciones.t3rsc.co" || route("home") == "soluciones.t3rsc.co"){

            if($requerimiento->getCargoEspecifico()->firma_digital == 1){
    ...
    */
    ...
    public function lista_sst(Request $data)
    {
        //dd($data->codigo);
        $candidatos = DatosBasicos::join("procesos_candidato_req", "procesos_candidato_req.candidato_id", "=", "datos_basicos.user_id")
            ...
            ->select("procesos_candidato_req.proceso", "procesos_candidato_req.id as ref_id", "datos_basicos.*", "requerimiento_cantidato.*","requerimiento_cantidato.id as req_can_id")
            ->paginate(5);

        return view("admin.reclutamiento.evaluaciones_sst", compact("candidatos"));
    }
    
    public function evaluacion_sst(Request $data)
    {
      ...
      $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
        ...
        return view("home.evaluacion_sst",compact('candidato'));
    }
    
    public function generar_carnet($id)
    {
        $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
                                 ->join("users","users.id","=","datos_basicos.user_id")
                                 ->where("requerimiento_cantidato.id", $id)
                                 ->select("datos_basicos.*", "requerimiento_cantidato.id as req_candidato","requerimiento_cantidato.requerimiento_id as req","users.foto_perfil","users.avatar")
                                 ->first();
        ...
        $req = Requerimiento::join("negocio", "negocio.id", "=", "requerimientos.negocio_id")
            ...
            ->leftjoin("requerimiento_contrato_candidato", "requerimiento_contrato_candidato.requerimiento_id", "=", "requerimientos.id")
            ->where("requerimiento_contrato_candidato.candidato_id", $candidato->user_id)
            ->whereRaw('requerimiento_contrato_candidato.id=(select max(requerimiento_contrato_candidato.id) from requerimiento_contrato_candidato where requerimiento_contrato_candidato.candidato_id= ? AND requerimiento_contrato_candidato.requerimiento_id=requerimientos.id)', [$candidato->user_id])
            ->select(
                "cargos_especificos.descripcion as cargo",
                "requerimientos.id",
                ...
                "clientes.logo as logo_cliente",
                "clientes.id as id_cliente",
                "requerimientos.id as req_id",
                "requerimiento_contrato_candidato.fecha_ingreso as fecha_inicio_contrato")
            ->groupBy('requerimientos.id')
            ->orderBy('requerimiento_contrato_candidato.id', "DESC")->find($candidato->req);

      //  $motivos = ["" => "Seleccionar"] + MotivosRechazos::pluck("descripcion", "id")->toArray();
        
        $qrcode = base64_encode(\QrCode::format('png')->size(200)->errorCorrection('H')->generate(route('informacionTrabajador', ['id' => $candidato->user_id])));
      
          $view = \View::make('home.carnet_candidato',compact('candidato','req','contrato', 'qrcode'))->render();
          $pdf =  app('dompdf.wrapper');
          $pdf->loadHTML($view);

          //return view('home.carnet_candidato',compact('candidato','req','contrato', 'qrcode'));
        return $pdf->stream('carnet.pdf');
    }
    
    public function confirmar_evaluacion_sst(Request $data)
    {
      ...
      $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
       ...
       return response()->json(["success" => true, "text_estado" => $estado->descripcion, 'candidato_req' => $data->get("candidato_req"),"id_proceso"=>$id_proceso,"view"=>'Enviado...']);
      //  return view("home.evaluacion_sst");
    }
    
    public function gestionar_evaluacion_sst($candidato)
    {
      $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
        ->where("requerimiento_cantidato.id", $candidato)
        ->select("datos_basicos.*", "requerimiento_cantidato.id as req_candidato", "requerimiento_cantidato.requerimiento_id")
        ->first();

      $evaluaciones = EvaluacionSST::where('id_candidato',$candidato->user_id)->where('id_req',$candidato->requerimiento_id)
                                    ->orderBy('puntuacion',"DESC")->get();

       // $motivos = ["" => "Seleccionar"] + MotivosRechazos::pluck("descripcion", "id")->toArray();
      return view("admin.reclutamiento.gestionar_evaluacion_sst", compact('candidato','evaluaciones'));
    }
    
    public function realizar_evaluacion_sst()
    {
        if(empty($this->user->id)){
         return redirect()->route('login', ['ev_sst' => true]);
        }
        return redirect()->route('dashboard')->with('no_prueba', 'Solicita otro enlace al psicólogo.');
         ...
         $candidatos = DatosBasicos::join("procesos_candidato_req", "procesos_candidato_req.candidato_id", "=", "datos_basicos.user_id")
            ...
            ->join('estados_requerimiento', 'requerimientos.id', '=', 'estados_requerimiento.req_id')
            ->whereRaw("(procesos_candidato_req.apto is null or  procesos_candidato_req.apto = '')")
            ->whereRaw('requerimiento_cantidato.id=(select max(requerimiento_cantidato.id) from requerimiento_cantidato where  requerimiento_cantidato.candidato_id=datos_basicos.user_id)')
            //->whereIn("requerimiento_cantidato.estado_candidato",[7, 8])
            ->whereIn("procesos_candidato_req.estado", [7, 8])
            //->whereRaw('estados_requerimiento.id=(select max(estados_requerimiento.id) from estados_requerimiento where estados_requerimiento.req_id=requerimientos.id)')
            //->whereIn("estados_requerimiento.estado", [                
       ...
       return view("home.modal.evaluacion_sst", compact('candidatos','permiso'));
    }
    ...
    public function guardar_evaluacion_sst(Request $data)
    {
       ...
       // dd($data->all());
        $campos = [
            ...
            'id_req'           => $candidato->requerimiento_id,
            "preg_uno"         => $data->resp1,
            "preg_dos"         => $data->resp2,
            "preg_tres"        => $data->resp3,
            "preg_cuatro"      => $data->resp4,
            "preg_cinco"       => $data->resp5,
            "preg_seis"        => $data->resp6,
            "preg_siete"       => $data->resp7,
            "preg_ocho"        => $data->resp8,
        ];

         $nueva = new EvaluacionSST();
         $nueva->fill($campos);
          
         $puntos = FuncionesGlobales::puntuacion_sst($nueva);
         
         if($puntos < 80){ //evaluar si es 80 puntos
          $mensaje = "Desafortunadamente obtuviste una calificación del (".$puntos."%) y no pasaste la evaluación, pues la calificación mínima es del 80%! \n Por favor haz clic en “Reintentar Evaluación” para realizarla de nuevo. Muchos éxitos!";
          $paso = 0;
          //return redirect()->back()->with("mensaje_success",$mensaje);
         }else{

           $nueva->puntuacion = $puntos;
           $nueva->save();

           $ref_id = RegistroProceso::where("requerimiento_candidato_id",$data->get("candidato_req"))->where('proceso',"ENVIO_SST")->orderBy('id',"DESC")->first();

           $paso = 1;
           $mensaje = "Evaluación finalizada con éxito,tu puntuación fue del ".$puntos."%,le informaremos al personal de selección que superaste la evaluación exitosamente!";
        
        } 
       
       return response()->json(["success" => true, "paso" => $paso, 'mensaje' => $mensaje,"id"=>$nueva->id]);
    ...
    {
       // dd($data->all());
        $resultados = EvaluacionSST::find($data->id_evaluacion);
        $resultados->firma = $data->firma;
        $resultados->save();

        $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
        ->where("requerimiento_cantidato.candidato_id", $resultados->id_candidato)
        ->select("datos_basicos.*","requerimiento_cantidato.id as req_can_id","requerimiento_cantidato.requerimiento_id")
        ->orderBy('id',"DESC")
        ->first();

        $ref_id = RegistroProceso::where("requerimiento_id",$resultados->id_req)->where("candidato_id",$resultados->id_candidato)
                                 ->where('proceso',"ENVIO_SST")->orderBy('id',"DESC")->first();
          // $ref_id->usuario_terminacion = ;
         $ref_id->apto = 1;
        ...
        $ruta = route('pdf_evaluacion_sst_anterior',$resultados->id);

        //correo de finalizacion de evaluacion sst
        $mailTemplate = 2; //Plantilla con botón e imagen de fondo
        $mailConfiguration = 1; //Id de la configuración
        $mailTitle = "Notificación de finalización de evaluación sst en REQ #{$resultados->id_req}"; //Titulo o tema del correo
        ...
        $mailBody = "
                    Buen dia,
                    <br/><br/>
                    El candidato <b>{$nombre_completo}</b> en el req <b>#{$resultados->id_req}</b>, ha finalizado su evaluación sst.
                    ";
        ...
        Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use($candidato, $envio) {

                $message->subject("Finalización Evaluación SST")
                        ->to([$envio->email],'soluciones')
                        ->getHeaders()
                        ->addTextHeader('X-SES-CONFIGURATION-SET', 'watch_emails_t3rs');
            });
    ...
    //pdf evaluacionsst
    public function pdf_evaluacion_sst($id, Request $data)
    {  
      // dd($data->all());
        $candidatos = EvaluacionSST::join('datos_basicos','datos_basicos.user_id','=','evaluacion_sst.id_candidato')
                                   ->select('datos_basicos.*','evaluacion_sst.*','evaluacion_sst.id as id_eva')->find($id);
         
        $view = \View::make("home.pdf_evaluacion_sst", compact('candidatos'))->render();
        $pdf  = \App::make('dompdf.wrapper');
        $pdf->loadHTML($view);
        $pdf->setPaper("A4");

           $output = $pdf->output();
           $nombre_documento = 'evaluacion_sst_'.$this->user->id.'.pdf';
    ...
    public function gestionar_referencia($id)
    {
    ...
    public function gestionar_referencia_estudios($id)
    {
        $candidato = DatosBasicos::join("procesos_candidato_req", "procesos_candidato_req.candidato_id", "=", "datos_basicos.user_id")
        ->join("requerimiento_cantidato", "requerimiento_cantidato.id", "=", "procesos_candidato_req.requerimiento_candidato_id")
        ->whereRaw(" (procesos_candidato_req.apto is null or procesos_candidato_req.apto = '' or procesos_candidato_req.apto = 3 )")
        ...
        ->leftJoin("motivos_rechazos", "motivos_rechazos.id", "=", "procesos_candidato_req.motivo_rechazo_id")
        ->where("requerimiento_candidato_id", $candidato->requerimiento_candidato_id)
        ->whereIn("proceso", ["ENVIO_REFERENCIA_ESTUDIOS", "ENVIO_REFERENCIA_ESTUDIOS_PENDIENTE"])
        ->get();
    ...
    public function seguimiento_candidato(Request $data)
    {
        ...
        $req_can = $data->get("candidato_req");
        ...
        ->select(
            ...
            'procesos_candidato_req.user_autorizacion',
            'procesos_candidato_req.puntuacion_sst',
            DB::raw("(select documentos.nombre_archivo from documentos where user_id=$candidato_id and documentos.tipo_documento_id=24 and requerimiento=requerimientos.id order by documentos.id desc limit 1) as nombre_archivo"),
            DB::raw("(select orden_medica.observacion from orden_medica where orden_medica.req_can_id=$req_can order by orden_medica.id desc limit 1) as observacion_medica"),
            DB::raw("(select orden_medica.id from orden_medica where orden_medica.req_can_id=$req_can order by orden_medica.id desc limit 1) as id_orden"),
    ...
    public function terminar_requerimiento(Request $campos)
    {
        ...
        $requerimiento = Requerimiento::find($campos->req_id);
        $estados_mostrar = [];
    ...
    public function pdf_informe_seleccion_req($user_id, Request $data){
       ...
       $reqcandidato = ReqCandidato::join("requerimientos", "requerimientos.id", "=", "requerimiento_cantidato.requerimiento_id")
        ...
        ->join("proceso_requerimiento", "proceso_requerimiento.entidad_id", "=", "entrevista_semi.id")
    ...
    public function pdf_informe_seleccion($user_id, Request $data)
    {
        ...
        if (route('home') == "http://localhost:8000") {
            ...
            ->select(
                ...
                "generos.descripcion as genero",
                "grupos_familiares.profesion_id as profesion",
                \DB::raw("CONCAT(grupos_familiares.nombres,' ',grupos_familiares.primer_apellido,' ',grupos_familiares.segundo_apellido) AS nombres_familiar"))
            ->where("grupos_familiares.user_id", $reqcandidato->candidato_id)
            ->orderBy("parentescos.id", "ASC")
            ->get();
        }
    ...
    public function reenviar_correo_video_confirmacion(Request $request)
    {
        ...
        if(!empty($email_doc_contrato->email)){
            Mail::send('admin.email-documentos-video-confirma', [
                ...
                $message->to($email_doc_contrato->email, "T3RS")
    ...
    public function editar_informacion_contratacion(Request $data)
    {
        ...
        ->select(
            ...
            "procesos_candidato_req.centro_costos as centro_costos"
        )
        ->groupBy('procesos_candidato_req.candidato_id')
        ->orderBy('requerimiento_cantidato.id', 'DESC')
        ->first();

        $centros_costos_proceso  = RegistroProceso::where("requerimiento_candidato_id", $data->get("candidato_req"))
        ->where("proceso", "ENVIO_CONTRATACION")
        ->orderBy('created_at', 'DESC')
        ->first();

        logger(json_encode($dato_contrato));
    ...
    public function guardar_informacion_contratacion(Request $data)
    {
        ...
        if(route('home') == "https://vym.t3rsc.co" || route('home') == "https://listos.t3rsc.co" ||
            ...
            $rules += [
                "fecha_ingreso_contra" => "required",
                "entidad_eps"           => "required",
                "entidad_afp"           => "required",
                "centro_costos"         => "required"
        ...
        if(route("home") == "https://vym.t3rs.co" || route("home") == "http://localhost:8000" ||

            route("home") == "https://listos.t3rs.co" || route("home") == "https://pruebaslistos.t3rs.co" ||
            route("home") == "https://soluciones.t3rsc.co" || route("home") == "https://pruebassoluciones.t3rsc.co"){
    ...
    public function reenviar_a_contratar(Request $data)
    {
        ...
        if(route("home") == "http://localhost:8000" || route("home") == "https://desarrollo.t3rsc.co" ||
            ...
            route("home") == "https://vym.t3rsc.co" || route("home") == "https://soluciones.t3rsc.co") {
        ...
        $newEndingDate = date("Y-m-d", strtotime(date("Y-m-d", strtotime($dato_contrato->fecha_ingreso)) . " + 365 day"));
    ...
    public function reenviar_a_contratar_proceso(Request $data)
    {
        ...
        if(route("home") == "https://desarrollo.t3rsc.co"){
            ...
            $campos = [
                ...
                'usuario_envio'              => $this->user->id,
                "fecha_inicio"               => date("Y-m-d H:i:s"),
                "fecha_inicio_contrato"      => $data->get("fecha_ingreso_contra"),
                "observaciones"              => $data->get("observaciones"),
                "centro_costos"              => $data->get("centro_costos"),
                "user_autorizacion"          => $data->get("user_autorizacion"),
                "usuario_terminacion"        => $this->user->id,
        ...
        if(route("home") == "http://localhost:8000" || route("home") == "https://desarrollo.t3rsc.co" ||
            route("home") == "https://listos.t3rsc.co" || route("home") == "https://vym.t3rsc.co" ||
            route("home") == "https://soluciones.t3rsc.co"){
