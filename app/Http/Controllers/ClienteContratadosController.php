...
class ClienteContratadosController extends Controller
{
    ...
    public function documentos_seleccion($candidato, $req, $req_can)
    {
        ...
        foreach ($tipo_documento as $key => &$tipo_doc) {
            $tipo_doc->documentos = DB::table('documentos')->select('nombre_archivo as nombre')->where('user_id', $candidato_id)->where('tipo_documento_id', $tipo_doc->id)->latest()->limit(5)->get();
        }
        unset($tipo_doc);
    ...
    public function documentos_confidenciales($candidato, $req)
    {
        ...
        return view('req.contratacion.documentos_confidenciales_new', compact(
            "tipo_documento",
    ...
    public function documentos_post($candidato, $req)
    {
        ...
        foreach ($tipo_documento as $key => &$tipo_doc) {
            $tipo_doc->documentos = DB::table('documentos')->select('nombre_archivo as nombre')->where('user_id', $candidato_id)->where('tipo_documento_id', $tipo_doc->id)->where('requerimiento', $req)->latest('id')->limit(5)->get();
        }
        unset($tipo_doc);
        ...
        return view('req.contratacion.documentos_post_new', compact(
            "tipo_documento",
            "getChecked",
            "datos_candidato",
            "requerimiento",
