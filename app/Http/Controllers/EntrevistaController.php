...
class EntrevistaController extends Controller
{
    ...
    public function gestionar_entrevista($id)
    {
        ...
        // $tiene_entrevistas_semi = EntrevistaSemi::join("users", "users.id", "=", "entrevista_semi.user_gestion_id")
        // ->where("entrevista_semi.candidato_id", $candidato->user_id)
        // ->where("entrevista_semi.req_id", $candidato->req_id)
        // ->select("entrevista_semi.*", "users.name")
        // ->first();

        $tiene_asociado_proceso = ProcesoRequerimiento::whereIn("tipo_entidad", ["MODULO_ENTREVISTA_SEMIESTRUCTURDA","MODULO_ENTREVISTA"])
        ...
        return view("admin.reclutamiento.gestionar_entrevista", compact(
            ...
            "entrevistas_semi",
            //"tiene_entrevistas_semi",
            "tiene_asociado_proceso"
        ));
    }
