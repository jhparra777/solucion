...
use App\Models\DiasTeletrabajo;
...
use App\Models\PorcentajeHvEvent;
use App\Models\SolicitudRecursos;
use App\Models\DiasTrabajoOficina;
...
use App\Models\HorarioTeletrabajo;
use App\Models\TipoTeletrabajador;
...
use App\Models\DiasSemanaTeletrabajo;
use App\Models\EstadosRequerimientos;
use Illuminate\Support\Facades\Event;
use App\Models\DescansoTeletrabajador;
...
class RequerimientoController extends Controller
{
    ...
    public function consultar_negocio_admin(Request $data)
    {
        ...
        ->select(
            "sociedades.division_nombre",
            "negocio.tipo_jornada_id",
            "negocio.nombre_negocio",
            "localidades.nombre as localidad",
            "tipo_negocios.nombre as tipo_negocio",
            "negocio.num_negocio",
            "negocio.cliente_id",
            "negocio.id",
            "negocio.sociedad"
    ...
    public function getCargoEspecificoDependientes(Request $data)
    {
        ...
        */
        ...
        $ficha_tipo_contrato      = ($cargo_especifico->prfl_tipo_contrato > 0 ? $cargo_especifico->prfl_tipo_contrato : $negocio->tipo_contrato_id);

        $ficha_jornada_laboral = ($cargo_especifico->prfl_jornada_laboral > 0 ? $cargo_especifico->prfl_jornada_laboral : $negocio->tipo_jornada_id);

        $tipo_liquidacion_id = ($cargo_especifico->prfl_tipo_liquidacion > 0 ? $cargo_especifico->prfl_tipo_liquidacion : $negocio->tipo_liquidacion_id);

        $tipo_salario_id = ($cargo_especifico->prfl_tipo_salario > 0 ? $cargo_especifico->prfl_tipo_salario : $negocio->tipo_salario_id);
        ...
        $sercon = ["" => "Seleccionar"] + Sercon::where('cliente_id', $data->cliente_id)->where("active", 1)
                                            ->select("id", DB::raw("CONCAT(codigo,' - ', descripcion) as descripcion"))
                                            ->orderBy("descripcion")->pluck("descripcion", "id")->toArray();

        $dias_semana_teletrabajo =  ["" => "Seleccionar"] + DiasSemanaTeletrabajo::where("active", 1)->pluck("descripcion", "id")->toArray();
        
        $dias_teletrabajo =  ["" => "Seleccionar"] + DiasTeletrabajo::where("active", 1)->pluck("descripcion", "id")->toArray();
        
        $dias_trabajo_oficina =  ["" => "Seleccionar"] + DiasTrabajoOficina::where("active", 1)->pluck("descripcion", "id")->toArray();
        
        $horario_teletrabajo =  ["" => "Seleccionar"] + HorarioTeletrabajo::where("active", 1)
                                                            ->select("id", DB::raw("CONCAT(dia,', ',tarde) as descripcion"))->pluck("descripcion", "id")->toArray();
        
        $tipos_teletrabajadores =  ["" => "Seleccionar"] + TipoTeletrabajador::where("active", 1)->pluck("descripcion", "id")->toArray();
        
        $descansos_teletrabajadores =  ["" => "Seleccionar"] + DescansoTeletrabajador::where("active", 1)->pluck("descripcion", "id")->toArray();
        ...
        if( $data->has('modulo') ){
            ...
            return response()->json([
                'dias_semana_teletrabajo'=> ($dias_semana_teletrabajo == 0 ? "" : $dias_semana_teletrabajo),
                'dias_teletrabajo'=> ($dias_teletrabajo == 0 ? "" : $dias_teletrabajo),
                'dias_trabajo_oficina'=> ($dias_trabajo_oficina == 0 ? "" : $dias_trabajo_oficina),
                'horario_teletrabajo'=> ($horario_teletrabajo == 0 ? "" : $horario_teletrabajo),
                'tipos_teletrabajadores'=> ($tipos_teletrabajadores == 0 ? "" : $tipos_teletrabajadores),
                'descansos_teletrabajadores'=> ($descansos_teletrabajadores == 0 ? "" : $descansos_teletrabajadores),
                'cargo_generico_id' => ($cargo_generico_id == 0 ? "" : $cargo_generico_id),
                'archivo'           => $archivo_perfil,
                'centro_trabajo'  => ($centro_trabajo == 0 ? "" : $centro_trabajo),
                'ctra_x_clt_codigo' => ($ctra_x_clt_codigo == 0 ? "" : $ctra_x_clt_codigo),
                'user_sesion' => $user_sesion,
                'centro_costos' => $centro_costos,
                'negocio' => $negocio,
                'tipo_liquidacion' => ($tipo_liquidacion == 0 ? "" : $tipo_liquidacion),
                'tipo_salario' => ($tipo_salario == 0 ? "" : $tipo_salario),
                'tipo_nomina' => ($tipo_nomina == 0 ? "" : $tipo_nomina),
                'concepto_pago' => ($concepto_pago == 0 ? "" : $concepto_pago),
                'salario' => $salario,
                'salario_min' => $salario_min,
                'salario_max' => $salario_max,
                'tipo_contrato' => ($tipo_contrato == 0 ? "" : $tipo_contrato),
                'tco_codigo' => $tco_codigo,
                'motivo_requerimiento' => ($motivo_requerimiento == 0 ? "" : $motivo_requerimiento),
                'motivo_requerimiento_id' => ($motivo_requerimiento_id == 0 ? "" : $motivo_requerimiento_id),
                'observaciones'  => $observaciones,
                'nivel_estudio' => ($nivel_estudio == 0 ? "" : $nivel_estudio),
                'nivel_estudio_id' => $nivel_estudio_id,
                'funciones' => $funciones,
                'edad_minima_selected' => $edad_minima_selected,
                'edad_maxima_selected' => $edad_maxima_selected,
                'generos' => ($generos == 0 ? "" : $generos),
                'genero_selected' => $genero_selected,
                'estados_civiles' => ($estados_civiles == 0 ? "" : $estados_civiles),
                'estado_civil_selected' => ($estado_civil_selected == 0 ? "" : $estado_civil_selected),
                'tipo_jornada' => ($tipo_jornada == 0 ? "" : $tipo_jornada),
                'ficha_jornada_laboral' => ($ficha_jornada_laboral == 0 ? "" : $ficha_jornada_laboral),
                'ficha_tipo_contrato' => ($ficha_tipo_contrato == 0 ? "" : $ficha_tipo_contrato),
                'ficha_numero_vacante'  => $ficha_numero_vacante,
                'ficha_nivel_estudio' => ($ficha_nivel_estudio == 0 ? "" : $ficha_nivel_estudio),
                ...
                'ficha_edad_maxima' => $ficha_edad_maxima,
                'ficha_genero' => ($ficha_genero == 0 ? "" : $ficha_genero),
                'tipo_experiencia' => ($tipo_experiencia == 0 ? "" : $tipo_experiencia),
                'tipo_experiencia_id' => ($tipo_experiencia_id == 0 ? "" : $tipo_experiencia_id),
    ...
    public function nuevo_requerimiento_admin($cliente_id, $negocio_id, Request $data)
    {
        ...
        if(route('home') != "http://tiempos.t3rsc.co" && route('home') != "https://tiempos.t3rsc.co"){
            //cliente_id 383 corresponde a ECOPETROL S.A.
            if ($cliente_id == 383) {
                $tipoProceso = ["" => "Seleccionar"] + TipoProceso::where("active", 1)->orderBy("descripcion", "asc")->pluck("descripcion", "id")->toArray();
            }else{
                $tipoProceso = ["" => "Seleccionar"] + TipoProceso::where("active", 1)->whereNotIn('id',[10,11])->orderBy("descripcion", "asc")->pluck("descripcion", "id")->toArray();
            }  
        ...
        $sercon = ["" => "Seleccionar"] + Sercon::where('cliente_id', $cliente_id)
        ->where("active", 1)
                                            ->select("id", DB::raw("CONCAT(codigo,' - ', descripcion) as descripcion"))
                                            ->orderBy("descripcion")
                                            ->pluck("descripcion", "id")
        ...
        return view("admin.nuevo_requerimiento_new", compact("user_sesion",
            "dias_semana_teletrabajo",
            "dias_teletrabajo",
            "dias_trabajo_oficina",
            "horario_teletrabajo",
            "tipos_teletrabajadores",
            "descansos_teletrabajadores",
            "psicologos",
            "centro_costo",
            "hay_atributos",
            "estructura_attr",
    ...
    {   
        ...
        /* la empresa contrata es soluciones porque es solo la que contrata en este cliente */
    ...
    public function editar_requerimiento($requerimiento_id, Request $data)
    {
        if(route("home") == "http://komatsu.t3rsc.co" || route("home") == "https://komatsu.t3rsc.co"){
            ...
            $tipo_nomina_id = $requermiento->tipo_nomina;
            ...
            $tipoTrabajo = ["" => "Seleccionar"] + TipoTrabajo::where("tipo_contrato_id", $requermiento->tipo_contrato_id)
                ->pluck("descripcion", "id")
                ->toArray();
            ...
            $sercon = ["" => "Seleccionar"] + Sercon::where('cliente_id', $negocio->cliente_id)->where("active", 1)
                        ->select("id", DB::raw("CONCAT(codigo,' - ', descripcion) as descripcion"))
                        ->orderBy("descripcion")->pluck("descripcion", "id")->toArray();
    ...
    {   
                ...
                $requerimiento->tipo_trabajo_id = $data->tipo_trabajo_id;
                ...
                $requerimiento->sercon_id       = $data->get("sercon_id");
                $requerimiento->dias_semana_teletrabajo_id = ($data->tipo_contrato_id == 6) ? $data->get("dias_semana_teletrabajo_id") : null;
                $requerimiento->horario_teletrabajo_id   = ($data->tipo_contrato_id == 6) ? $data->get("horario_teletrabajo_id") : null;
                $requerimiento->tipo_teletrabajador_id   = ($data->tipo_contrato_id == 6) ? $data->get("tipo_teletrabajador_id") : null;
                $requerimiento->descanso_teletrabajador_id  = ($data->tipo_contrato_id == 6) ? $data->get("descanso_teletrabajador_id") : null;
                $requerimiento->dias_teletrabajo_id  = ($data->tipo_contrato_id == 6) ? $data->get("dias_teletrabajo_id") : null;
                $requerimiento->dias_trabajo_oficina_id = ($data->tipo_contrato_id == 6) ? $data->get("dias_trabajo_oficina_id") : null;
                $requerimiento->save();
    ...
    public function guardar_requerimiento_copia(Request $data){
        ...
        for ($i = 0; $i < count($ciudadReqs); $i++) {
            ...
            if (route('home') != "http://komatsu.t3rsc.co") {
                ...
                $nuevoRequerimiento->adicionales_salariales        = $copia->adicionales_salariales;
                $nuevoRequerimiento->sercon_id                     = $copia->sercon_id;
                
                $nuevoRequerimiento->dias_semana_teletrabajo_id = $copia->dias_semana_teletrabajo_id;
                $nuevoRequerimiento->horario_teletrabajo_id   = $copia->horario_teletrabajo_id;
                $nuevoRequerimiento->tipo_teletrabajador_id   = $copia->tipo_teletrabajador_id;
                $nuevoRequerimiento->descanso_teletrabajador_id  = $copia->descanso_teletrabajador_id;
                $nuevoRequerimiento->dias_teletrabajo_id  = $copia->dias_teletrabajo_id;
                $nuevoRequerimiento->dias_trabajo_oficina_id = $copia->dias_trabajo_oficina_id;
    ...
    public function trazabilidad_cliente(Request $data){
        ...
        return view("req.modals.trazabilidad_candidato_cliente_new", compact("candidatos_req", "requerimiento", "cliente"));
    }
    ...
    function tiposTrabajosPorTipoContrato(Request $request)
    {
        $tipos = TipoTrabajo::where("tipo_contrato_id", $request->tipo_contrato_id)
        ->pluck("descripcion", "id")
        ->toArray();
