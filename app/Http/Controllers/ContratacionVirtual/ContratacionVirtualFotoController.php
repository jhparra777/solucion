...
class ContratacionVirtualFotoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $firma_contrato = FirmaContratos::where('user_id', $user_id)->where('req_id', $req_id)->select('id')->orderBy('id', 'DESC')->first();

        $estado = $request->camara;

        $adicionales_fotos_count = $this->adicionales_asociados($req_id, $firma_contrato->id);
    ...
    private function adicionales_asociados($req_id, $contrato_id)
    {
        $adicionales = DocumentoAdicional::join('cargos_documentos_adicionales', 'cargos_documentos_adicionales.adicional_id', '=', 'documentos_adicionales_contrato.id')
        ->join('requerimientos', 'requerimientos.cargo_especifico_id', '=', 'cargos_documentos_adicionales.cargo_id')
