...
class ContratacionController extends Controller
{
    ...
    */
    public function index(Request $data)
    {
        ...
        ->select(
            ...
            DB::raw('(select id from estudios where user_id=users.id limit 1 ) as tiene_estudio')
        )
        ->with("procesos")
    ...
    public function gestionar_candidato($candidato,$req)
    {
        ...
        if($cantidad_doc_contratacion != 0){
    ...
    */
    public function documentos_contratacion($candidato, $req)
    {
        ...
        $sst = EvaluacionSST::where('id_candidato',$candidato)->where('id_req',$req)->orderBy('id','DESC')->first();
    ...
    */
    ...
    public function cargar_documento_admin_beneficiario(Request $request)
    {
        ...
        return view("admin.contratacion.candidato.modal.nuevo_documento_beneficiario_asistente", compact("tipos_documentos", "gruposFamiliares", "cand_id", "req_id"));
    }
    ...
    public function guardar_documento_asistente_post(Request $data)
    {
        ...
        if($data->hasFile('archivo_documento') /*&& !$validator->fails()*/){
            $cant_archivos = 0;
            $cant_no_procesados = 0;
            $validas   = array("pdf", "png", "docx", "doc", "jpg", "jpeg","xls","xlsx");
            $datosU = DatosBasicos::where('user_id', $data->cand_id)->first();
            $desc = TipoDocumento::where("id", $data->tipo_documento_id)->pluck('descripcion');
            ...
            if ($cant_no_procesados == $cant_archivos) {
                ...
                $users = User::join('users_x_clientes','users_x_clientes.user_id','=','users.id')             ->join('role_users','role_users.user_id','=','users.id')            
                     ...
                     ->select("users.email as email", "users.name", "clientes.nombre as cliente_nombre", "users.id")
                     ->groupBy("users.id")
                     ->get();
                ...
                foreach ($users as $usuario) {
                    ...
                    $mailBody = "
                                Hola {$usuario->name}: 
                                <br/><br/>
                                Te informamos que se ha cargado el documento {$desc} a la carpeta de Post-Contratación del candidato {$nombre_completo} en el requerimiento {$data->req_id} del cliente {$usuario->cliente_nombre}.
                    ...
                    $mailButton = ['buttonText' => 'Visualizar documento', 'buttonRoute' => route("admin.documentos_post", ["candidato" => $datosU->user_id, "req" => $data->req_id])];
    ...
    public function cargar_documento_admin_post(Request $request)
    {
        ...
        $tipo_documento = ["" => "Seleccionar"] + TipoDocumento::where("categoria", 4)
        ->where("tipos_documentos.estado", 1)
        ->orderBy("tipos_documentos.descripcion")
        ->pluck("tipos_documentos.descripcion", "tipos_documentos.id")
        ->toArray();
    ...
    public function ver_status_contratacion(Request $request)
    {
        ...
        ->select(
            ...
            DB::raw("(select documentos.nombre_archivo from documentos where user_id=$user_id and documentos.tipo_documento_id=24 order by documentos.id desc limit 1) as nombre_archivo"),
    ...
    public function envio_documentos_contratacion(Request $data)
    {
        ...
        $requerimiento_contrato_candidato = RequerimientoContratoCandidato::select('centro_costo_id')
            ->where('requerimiento_id', $req)
            ->where('candidato_id', $user->id)
            ->orderBy('id', 'desc')
        ->first();

        if (is_null($requerimiento_contrato_candidato)) {
            $requerimiento->centro_costo = 0;
        } else {
            $requerimiento->centro_costo = $requerimiento_contrato_candidato->centro_costo_id;
        }
        ...
        if ($data->carnet != null) {
            ...
            $req_carnet = Requerimiento::join("negocio", "negocio.id", "=", "requerimientos.negocio_id")
                ...
                ->where("requerimiento_contrato_candidato.candidato_id", $candidato_carnet->user_id)
                ->select(
                    "cargos_especificos.descripcion as cargo",
                    "requerimientos.id",
            ...
            /* para codigo QR */
            $qrcode = base64_encode(\QrCode::format('png')->size(200)->errorCorrection('H')->generate(route('informacionTrabajador', ['id' => $user->id])));
        ...
        try {
            ...
            foreach(json_decode($data->candidato_req) as $key => $req_candi_id) {
                ...
                if ($se_puede_contratar == 'si') {
                    ...
                    $centro_costo_id = (isset($requerimiento_cand->centro_costo_id) && $requerimiento_cand->centro_costo_id != null ? $requerimiento_cand->centro_costo_id : 1);
                    ...
                    $historialContratacion->centro_costo_id = $centro_costo_id;     //Solo Soluciones
                    ...
                    //$historialContratacion->arl_id = $data->arl;   Soluciones no lleva ARL
            ...
            return response()->json(["success" => false, "view" => 'Ha ocurrido un error al procesar. Intente nuevamente, si el problema persiste contacte con soporte.']);
        }
    }
