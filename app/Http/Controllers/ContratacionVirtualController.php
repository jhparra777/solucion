...
class ContratacionVirtualController extends Controller {
    ...
    */
    public function firmaContratoCandidato($user_id, $req_id, $modulo = 'modulo_cv')
    {
        ...
        ->leftJoin("datos_aprendiz", "datos_aprendiz.datos_basicos_id", "=", "datos_basicos.id")
        ...
        ->select(
            "datos_basicos.*",
            "datos_aprendiz.especialidad",
            "datos_aprendiz.numero_grupo",
            "datos_aprendiz.institucion",
            "datos_aprendiz.sena_centro_formacion",
        ...
        ->leftJoin('tipos_trabajos', 'tipos_trabajos.id', '=', 'requerimientos.tipo_trabajo_id')
        ->leftJoin("sercon", "sercon.id", "=", "requerimientos.sercon_id")
        ->leftJoin("dias_semana_teletrabajo", "dias_semana_teletrabajo.id", "=", "requerimientos.dias_semana_teletrabajo_id")
        ->leftJoin("horario_teletrabajo", "horario_teletrabajo.id", "=", "requerimientos.horario_teletrabajo_id")
        ->leftJoin("tipos_teletrabajadores", "tipos_teletrabajadores.id", "=", "requerimientos.tipo_teletrabajador_id")
        ->leftJoin("descansos_teletrabajadores", "descansos_teletrabajadores.id", "=", "requerimientos.descanso_teletrabajador_id")
        ->leftJoin("dias_teletrabajo", "dias_teletrabajo.id", "=", "requerimientos.dias_teletrabajo_id")
        ->leftJoin("dias_trabajo_oficina", "dias_trabajo_oficina.id", "=", "requerimientos.dias_trabajo_oficina_id")
        ->select(
            ...
            "sercon.codigo as codigo_sercon",
            "sercon.descripcion as descripcion_sercon",
            "tipos_trabajos.descripcion as tipo_trabajo",
            "dias_semana_teletrabajo.descripcion as dia_semana",
            "horario_teletrabajo.dia as horario_dia",
            "horario_teletrabajo.tarde as horario_tarde",
            "tipos_teletrabajadores.descripcion as tipo_teletrabajador",
            "descansos_teletrabajadores.descripcion as descanso_teletrabajador",
            "dias_teletrabajo.descripcion as dias_teletrabajar",
            "dias_trabajo_oficina.descripcion as dias_trabajar_oficina"
        )
        ...
        */
        ...
        if($sitio->multiple_empresa_contrato) {
            ...
            /*return view("contratos.".$tipo_contrato->modelo_contrato, compact(
                'user_id',
                'req_id',
                'userId',
                'ReqId',
            ...
            /*return view("home.firma-contrato-candidato", compact(
                'user_id',
                'req_id',
                'userId',
                'ReqId',
    ...
    */
    public function confirmar_documentos_adicionales($user_id, $contrato_id, $moduloHash)
    {
        ...
        if ($modulo == 'modulo_cv') {
            if($userId != $this->user->id){
                return redirect()->route('home');
            }else if($checkContrato != null && count($checkContrato)){
                return redirect()->route('dashboard');
            }
        } else {
            //modulo_admin
            if(count($checkContrato != null && $checkContrato)){
                return redirect()->route('notfound');
            }
        }
        ...
        */
        ...
        if (count($documentos_parametrizados_creado) > 0) {
            if ($documentoCreado != null) {
                ...
                /*return view("home.firma-documento-adicional", compact(
                    'user',
                    'candidato',
                    'contratoId',
                    'documentoCreado',
        ...
        if($documento == null || count($documentos_parametrizados) <= 0) {
            ...
            if ($firmaContratoFin->terminado != 3) {
                ...
                if (count($contrato_fotos) > 0) {
                    ...
                    /*/Envío de correo Solo Listos -- Solo Listos
    ...
    */
    public function guardarFirma(Request $data)
    {
        ...
        ->select(
            ...
            "requerimiento_cantidato.created_at",
            "requerimiento_cantidato.requerimiento_id",
            "requerimientos.detalle_rodamiento",
            "requerimiento_cantidato.id as requerimiento_cantidato_id",
            "clientes.nombre as cliente_nombre",
            "procesos_candidato_req.fecha_inicio_contrato",
            "procesos_candidato_req.fecha_fin_contrato",
        ...
        $rodamiento = $reqcandidato->detalle_rodamiento; //evaluar detalle rodamiento

        $situacion_militar = NULL; // 0 situacion militar no definida // 1 situacion militar definida // null es mujer

        //verificar la situacion militar del candidato si es masculino
        $datoscand = DatosBasicos::where('user_id',$usuario_id)->first();

        if($datoscand->genero == 1){
            // si es genero masculino
            $situacion_militar = (!is_null($datoscand->situacion_militar_definida)) ? $datoscand->situacion_militar_definida : 0;
            // es no definida por defecto
        }
        ...
        ->where(function ($sql) use ($situacion_militar, $rodamiento){
            if($situacion_militar == 1){
                //si su situacion esta definoda excluir la clausula 2
                $sql->whereNotIn("documentos_adicionales_contrato.id",[2]);
            }

            if($situacion_militar == 0){
                //si su situacion no esta definoda excluir la clausula 3
                $sql->whereNotIn("documentos_adicionales_contrato.id",[3]);
            }

            if(is_null($situacion_militar)){
                //si no es masculino entonces excluir ambas clausulas
                $sql->whereNotIn("documentos_adicionales_contrato.id",[2,3]);
            }

            if(is_null($rodamiento) || $rodamiento == "" || $rodamiento <= 0){
                //si no tiene rodamiento entonces no pedir esa firma
                $sql->whereNotIn("documentos_adicionales_contrato.id",[4]);
            }
        })
        ->orderBy('documentos_adicionales_contrato.id', 'ASC')
        ->get();
        ...
        if($data->has('recomendaciones')){
            ...
            */
            ...
            if($data->has('creada')) {
                ...
                $view = \SnappyPDF::loadView($vista, ["firma" => $firma, "fecha" => $fecha, "user" => $user, "requerimiento" => $requerimiento, "lugarexpedicion" => $lugarexpedicion, "candidato" => $candidato])->output();
            }
    ...
    */
        ...
        if ($nro_pregunta == $cantidad_preguntas) {
            ...
            if ($terminadoAnterior == null) {
                ...
                ->select(
                    ...
                    "categorias_licencias.descripcion as categorias_licencias_des",
                    "entidades_afp.descripcion as entidades_afp_des",
                    "users.foto_perfil","users.avatar",
                    "entidades_eps.descripcion as entidades_eps_des"
                )
                ->first();
                ...
                if ($cuenta_contratos_firmados != null || $cuenta_contratos_firmados != '') {
                    if($cuenta_contratos_firmados >= $numero_vacantes){
                        ...
                        if($buscar_estado_req == null || $buscar_estado_req == ''){
                            ...
                            //correo al candidato contratado
                ...
                ->select(
                    ...
                    "motivo_requerimiento.descripcion as motivo_requerimiento",
                    "sercon.codigo as codigo_sercon",
                    "sercon.descripcion as decripcion_sercon",
                    "tipos_trabajos.descripcion as tipo_trabajo",
                    "dias_semana_teletrabajo.descripcion as dia_semana",
                    "horario_teletrabajo.dia as horario_dia",
                    "horario_teletrabajo.tarde as horario_tarde",
           ...
           /*
            //  $view2 = \View::make('home.carnet_candidato',compact('req','candidato'))->render();
            //  $pdf2  = \App::make('dompdf.wrapper');
            //  $pdf2->loadHTML($view);          
              //Guarda contrato
            //  $output2 = $pdf2->output();
            //  $carnet = 'carnet_candidato_'.$this->user->id.'.pdf';
            $fecha = date('d-m-Y');
             //Calcular edad de candidatos.
             $edad = ($candidato->fecha_nacimiento != 0 && $candidato->fecha_nacimiento != "") ? Carbon::parse($candidato->fecha_nacimiento)->age:"";
             ...
             $lugarresidencia = null;
              ...
              if($candidato != null) {
                ...
                //leftjoin("empresa_logos","empresa_logos.id","=","requerimientos.empresa_contrata")
                where("requerimientos.id", $ReqId)
                ->select("requerimientos.id as req_id","requerimientos.*","requerimientos.cargo_especifico_id as cargo_especifico_id")
                ->first();
             // file_put_contents('contratos/'.$carnet, $output2);
             ...
             $fechasContrato = RequerimientoContratoCandidato::where('requerimiento_id', $ReqId)
               ...
               $firmaContrato =  FirmaContratos::where('id', $contrato_id)
                ...
                $view = \View::make("home.pdf-contrato-firmar", compact(
                    'userId',
                    'ReqId',
                    'candidato',
                    'firmaContrato',
                ...
                //Guarda contrato
                $output = $pdf->output();
                $nombre_documento = 'contrato_'.$ReqId.'_'.$this->user->id.'.pdf';
                ...
                file_put_contents('contratos/'.$nombre_documento, $output);*/
                  ...
                  /*      Mail::send('admin.email_carnet', [
                            'candidato_id' => $candidato->req_candidato,
                            'candidato' => $candidato->nombres.' '. $candidato->primer_apellido.' '.$candidato->segundo_apellido,
                            'req' => $ReqId
                            ], function ($m) use($candidato,$firmaContratoFin) {
                                $m->subject("Contrato finalizado");
                                $m->to([$candidato->email],'soluciones');
                                $m->attach(public_path('contratos/'.$firmaContratoFin->contrato));
                                $m->attach(public_path('contratos/carnet_candidato_'.$this->user->id.'.pdf'));
                            });*/
    ...
    public function guardar_confirmacion_manual(Request $data)
    {
        ...
        //$pregunta_id = $data->pregunta_id;
        // dd($data->all());
        /*$question = ConfirmacionPreguntaContrato::where('user_id', $this->user->id)
        ->where('pregunta_id', $pregunta_id)
        ->where('contrato_id', $contrato_id)
        ->where('estado', 0)
        ->first();*/
    ...
    */
    public function anular_contratacion_candidato(Request $data)
    {
        ...
        $nuevo_proceso->fill([
            ...
            'requerimiento_id'           => $req_id,
            'candidato_id'               => $user_id,
            'observaciones'              => "El contrato fue anulado (Módulo admin).",
        ...
        /**
         * Si hay clausula médica la deja activa nuevamente
         */

        DB::table('orden_medica')
        ->where('req_can_id', $nuevo_estado->id)
        ->where('aceptada', 1)
        ->update(['aceptada' => null]);
        ...
        $empresa_contrata = (route('home') == 'https://listos.t3rs.co')?EmpresaLogo::where('id', $reqcandidato->empresa_contrata)->first():'';
        ...
        $requerimiento = Requerimiento::where("requerimientos.id", $req_id)
        ->select(
            "requerimientos.id as req_id",
            "requerimientos.*",
            "requerimientos.id",
        ...
        return response()->json([ 'success' => true, 'reenviar' => true ]);
    }
    ...
    */
    public function generateContract(Request $data, $user_id, $req_id, $email = null)
    {
        ...
        /*
        //Si en algun momento no se quieren mostrar las secciones de firma y de documentos adicionales
        if($data->has('preview')) {
            $mostrar_firma = 'NO';
            $mostrar_adicionales = 'NO';
        }
        */

        //Agregar clausulas
        if (!empty($reqcandidato_clausulas) && $mostrar_adicionales === 'SI') {
            //$adicionales = CargoDocumentoAdicional::where("cargo_id", $reqcandidato_clausulas->cargo_req)->get();

            //Crear confirmación de documentos
            //TODO: Revisar los documentos adicionales para el contrato enviado por correo o para descargar

            $rodamiento = $reqcandidato_clausulas->detalle_rodamiento; //evaluar detalle rodamiento

            $situacion_militar = NULL; // 0 situacion militar no definida // 1 situacion militar definida // null es mujer

            //verificar la situación militar del candidato si es masculino
            if($candidato->genero == 1){
                // si es género masculino
                $situacion_militar = (!is_null($candidato->situacion_militar_definida)) ? $candidato->situacion_militar_definida : 0;
                // es no definida por defecto
            }
            ...
            $adicionales = CargoDocumentoAdicional::join("documentos_adicionales_contrato", "documentos_adicionales_contrato.id", "=", "cargos_documentos_adicionales.adicional_id")
                ...
                ->orderBy('documentos_adicionales_contrato.id', 'ASC')
            ->get();
            //logger(json_encode($adicionales));
        ...
        ->select(
            ...
            "requerimientos.negocio_id",
            "requerimientos.cargo_especifico_id as cargo_especifico_id",
            "requerimientos.motivo_requerimiento_id",
            "requerimientos.tipo_liquidacion",
            "requerimientos.tipo_nomina",
            "requerimientos.tipo_salario",
            "requerimientos.tipo_contrato_id"
        )
        ->first();
