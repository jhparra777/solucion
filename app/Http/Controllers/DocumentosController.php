...
class DocumentosController extends Controller
{
    public function index()
    {
        ...
        if(route("home") == "https://komatsu.t3rsc.co"){
            $tipoDocumento = ["" => "Seleccionar"] + TipoDocumento::where("active", "1")->pluck("descripcion", "id")->except("id", 8, "id", 9)->toArray();
        }elseif(route("home") == "https://colpatria.t3rsc.co") {
            $tipoDocumento = ["" => "Seleccionar",1 => "CEDULA DE CIUDADANIA"];
        }else {
        ...
        ->where("documentos.user_id", $this->user->id)->get();

        return view("cv.documentos", compact("tipoDocumento", "documentos","menu"));
    }

    public function guardar_documento(Request $data, Requests\DocumentoNuevoRequest $validate)
    {
        $this->validate($data,[
            ""
        ]);
        if ($data->hasFile('archivo_documento') || $data->hasFile('foto-documento')) {
            ...
            if ($extencion == 'jpg' || $extencion == 'png' || $extencion == 'jpeg' || $extencion == 'pdf') {
                ...
                //$documentos->nombre_archivo = null;
                //$documentos->save();
            }
        }
        ...
        ->where("categoria", 1)
        ->where("carga_candidato",1)->pluck("descripcion", "id")
        ->toArray();
    ...
    public function editar_documento(Request $data)
    {
        ...
        //return response()->json(["view" => view("cv.modal.fr_documentos", compact("campos", "tipoDocumento", "editar"))->render()]);
    ...
    public function actualizar_documento(Request $data, Requests\DocumentoEditarRequest $validate)
    {
        ...
        /*$campos        = [];
        $tipoDocumento = ["" => "Seleccionar"] + TipoDocumento::where("active", "1")->pluck("descripcion", "id")->toArray();
        $mensaje       = "Se ha actualizado el documento sin errores";
        $documento     = Documentos::join("tipos_documentos", "tipos_documentos.id", "=", "documentos.tipo_documento_id")
        ->where("documentos.id", $documentos->id)
        ->select("documentos.*", "tipos_documentos.descripcion as tipo_doc")
        ->first();
    ...
    public function lista_documentos(Request $data)
    {
        ...
        }elseif(route("home") == "https://demo.t3rs.co"){
    ...
    public function lista_documentos_medicos(Request $data)
    {
        ...
        if (route("home") == "http://komatsu.t3rsc.co" || route("home") == "https://komatsu.t3rsc.co") {    
            ...
            ->where(function ($sql) use ($data,&$estados,&$search) {
                ...
                if ($data->cliente_id != "") {
                    $sql->where("clientes.id", $data->cliente_id);
                    $estados=[1,2,3,4,6];
                    $search=true;
                }
            ...
            ->select(
                ...
                "requerimientos.fecha_ingreso as fecha_tentativa_ingreso",
        ...
        return view("admin.reclutamiento.documentos_medicos", compact("candidatos","search", "clientes"));
    }

    public function cambiar_concepto_medico(Request $data)
    {
        

       $orden=$data->get("orden");
       
        return view("admin.reclutamiento.modal.cambiar_concepto_medico", compact("orden"));
    }

    public function confirmar_cambiar_concepto_medico(Request $data)
    {
       $orden=OrdenMedica::find($data->get('orden'));

        return redirect()->route('admin.gestionar_documentos_medicos',['ref_id'=>$orden->id]);
    ...
    public function gestionar_documentos($id)
    {
       $candidato = DatosBasicos::join("procesos_candidato_req", "procesos_candidato_req.candidato_id", "=", "datos_basicos.user_id")
        ...
        ->leftJoin("motivos_rechazos", "motivos_rechazos.id", "=", "procesos_candidato_req.motivo_rechazo_id")
        ->where("requerimiento_candidato_id", $candidato->requerimiento_candidato_id)
        ->whereIn("proceso", ["ENVIO_DOCUMENTOS", "ENVIO_DOCUMENTOS_PENDIENTE"])
        ->get();
        ...
        return view("admin.reclutamiento.gestionar_documentos", compact(
            "candidato",
            "pruebas",
    ...
    public function gestionar_documentos_medicos($id,$edit=0)
    {
        ...
        if($sitio_modulo->salud_ocupacional != "si"){
            ...
            $resultados=DB::table('orden_medica_resultados')->where("orden_id",$id)->leftjoin("users","users.id","=","orden_medica_resultados.user_gestion")->select("orden_medica_resultados.*","users.name as usuario_gestiona")->get();
    ...
    public function nuevo_documento_estudio_seguridad_verificado(Request $data)
    {
        ...
        $tipoDocumento = ["" => "Seleccionar"] + TipoDocumento::where("active", "1")
        ->where("descripcion", "ESTUDIO SEGURIDAD")
        ->pluck("descripcion", "id")
        ->toArray();

        if(route("home")=="http://komatsu.t3rs.co" || route("home")=="https://komatsu.t3rs.co" ||
            route("home")=="http://listos.t3rs.co" || route("home")=="https://listos.t3rs.co" ||
            route("home")=="http://vym.t3rs.co" || route("home")=="https://vym.t3rs.co"){    
    ...
    public function guardar_estudio_seguridad(Request $data)
    {
       ...
       if(route("home")=="http://localhost:8000" || route("home")=="https://demo.t3rs.co" || route("home")=="https://listos.t3rs.co" || route("home")=="https://soluciones.t3rsc.co"){
...
//dd($tipo);
    ...
    public function guardar_examen_medico(Request $data, Requests\DocumentoMedicoNuevoRequest $valida)
    {
        ...
        if ($data->hasFile('archivo_documento')) {
            ...
            $documentos->requerimiento = $req_can->requerimiento_id;
            $documentos->tipo_documento_id = 24;
        ...
        if($data->get("email_cliente")!="" && $data->get("email_cliente")!=null){
            $email_cliente=explode(',',str_replace(" ","",$data->get("email_cliente")));
            $observaciones=$data->get("observacion");
            if($data->has("seguimiento")){
                $seguimiento=SI;
            }
            else{
                $seguimiento=NO;
            ...
            try{
                ...
                $mailTitle = "RESULTADOS EVALUACIÓN MEDICA OCUPACIONAL"; //Titulo o tema del correo

                //Cuerpo con html y comillas dobles para las variables
                $mailBody = "<p>Apreciado (a) cliente $proceso->cliente</p>

                <p>Reciba un cordial saludo del área de Seguridad y Salud de Soluciones Inmediatas </p>

                <p style='text-align: justify;'>A continuación encontrarás las recomendaciones y/o restricciones emitidas en el certificado médico de la evaluación médica ocupacional de ingreso realizada al candidato <strong>$candidato->nombres $candidato->primer_apellido $candidato->segundo_apellido</strong> con ID <strong>$candidato->numero_id</strong> para el cargo <strong>$proceso->cargo</strong>.</p>

                <br>

                <p>Recomendaciones y/o restricciones:</p>

                <p>$observaciones</p>

                <br>

                <p>Entra en Seguimiento por SST SOLUCIONES INMEDIATAS:</p>
                ...
                <br>
                <br>
                <p>Si desea descargar el concepto médico del candidato lo podrá realizar en la hoja de vida después del proceso de contratación o con la psicóloga encargada</p>";

                //Arreglo para el botón
                $mailButton = [];

                $mailUser = 2; //Id del usuario al que se le envía el correo
                ...
                Mail::send($triEmail->view, ['data' => $triEmail->data], function ($m) use($candidato,$envio,$email_cliente) {
                    $m->subject("NOTIFICACIÓN RESULTADOS EVALUACIÓN MEDICA OCUPACIONAL(".$candidato->nombres." CC ".$candidato->numero_id.")")
                    ->to($email_cliente)
                    ->bcc("sstbogota4@sisaemp.com","SST")
                    ->getHeaders()
