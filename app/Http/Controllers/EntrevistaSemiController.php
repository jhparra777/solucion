...
class EntrevistaSemiController extends Controller
{
    ...
    public function nueva_entrevista(Request $data)
    {
        ...
        $estudios = Estudios::leftjoin("niveles_estudios", "niveles_estudios.id", "=", "estudios.nivel_estudio_id")
            ->leftjoin("paises", "paises.cod_pais", "=", "estudios.pais_estudio")
            ...
            ->select(
                "estudios.*",
                DB::raw("CONCAT(paises.nombre,' ',departamentos.nombre,' ',ciudad.nombre)  AS ciudad_completa_estudio"), 
        ...
        return view("admin.reclutamiento.modal.nueva_entrevista_semi", compact("trabajo","req_id","fuentes","proceso","estudios","experiencias","familiares","hijos","competencias", "estadoCivil", "parentesco", "motivoRetiro", "cargoGenerico"));
    ...
    public function guardar_entrevista(Request $data)
    {
        ...
        if(route('home') == "https://komatsu.t3rsc.co" || route('home') == "http://komatsu.t3rsc.co") {
            ...
            $this->validate($data, [
                ...
                'fortalezas' => 'required',
                'opor_mejora' => 'required',
                //'proyectos' => 'required',  //Eliminado en Abril 2021
                'valores' => 'required',
            ...
            //FIN logica Json
            $estado_civil = EstadoCivil::where('id', $data->estado_civil)->pluck('descripcion')->first();
            ...
            $nueva_entrevista->fill([
                ...
                'covid2'=>$data->covid2,
                'covid3'=>$data->covid3,
                'diagnosticado_covid19' => $data->diagnosticado_covid19,
                'fecha_diagnosticado_covid19' => $data->fecha_diagnosticado_covid19,
                'secuela_covid19' => $data->secuela_covid19,
                'cual_secuela_covid19' => $data->cual_secuela_covid19,
                'vacunado_contra_covid19' => $data->vacunado_contra_covid19,
                'cuantas_dosis_vacuna_covid19' => $data->cuantas_dosis_vacuna_covid19,
                'fechas_vacunas_covid19' => implode(",", $data->fecha_vacuna_covid19),
                'tiene_familiares'  => $data->tiene_familiares,
                'proyecto1'=>$data->proyecto1,
                'proyecto2'=>$data->proyecto2,
                'proyecto3'=>$data->proyecto3,
                'reporte'   =>$data->reporte,
                'nombres_apellidos'=> $data->nombres_apellidos,
                'cedula'=> $data->numero_id,
                ...
                'barrio'=> $data->barrio,
                'familiares_fanalca'=> $data->familiares_fanalca,
                'libreta_militar'=> $data->libreta_militar,
                'req_id'   =>$data->req_id,
         ...
         $parentesco=$data->parentesco;
            $nombre_vive=$data->nombre_vive;
            $edad_vive=$data->edad_vive;
            $ocupacion_vive=$data->ocupacion_vive;

        //INICIO de lógica para guardar JSON
            $vive_con=array();
            for($i=0;$i<count($parentesco);$i++){

                if($parentesco[$i]!='' && $nombre_vive[$i]!='' && $edad_vive[$i]!='' && $ocupacion_vive[$i]!=''){
                    
                    $parentesco_desc = Parentesco::where("id", $parentesco[$i])->pluck("descripcion")->first();
        
                    $vive=array("parentesco"=>$parentesco_desc,"nombre"=>$nombre_vive[$i],"edad"=>$edad_vive[$i],"ocupacion"=>$ocupacion_vive[$i]);

                    array_push($vive_con,$vive);
                }
            }

            $nueva_entrevista->vive_con=json_encode($vive_con);

            $nombre_hijo=$data->nombre_hijo;    
            $edad_hijo=$data->edad_hijo;
            $ocupacion=$data->ocupacion;
            $con_quien=$data->con_quien;
            $cada_cuanto=$data->cada_cuanto;
            $apoyo_economico=$data->apoyo_economico;
            $vive_hijos=array();
            for($i=0;$i<count($nombre_hijo);$i++){

                if($nombre_hijo[$i]!='' && $edad_hijo[$i]!='' && $ocupacion[$i]!=''){
                    $vive_h=array("nombre"=>$nombre_hijo[$i],"edad"=>$edad_hijo[$i],"ocupacion"=>$ocupacion[$i],"con_quien"=>$con_quien[$i],"cada_cuanto"=>$cada_cuanto[$i],"apoyo_economico"=>$apoyo_economico[$i]);

                    array_push($vive_hijos,$vive_h);
                }
            }


            $nueva_entrevista->hijos=json_encode($vive_hijos);
            

            //familiares en empresa
            $nombre_familiar = $data->nombre_familiar;
            $apellido_familiar = $data->apellido_familiar;
            $cargo_familiar = $data->cargo_familiar;
            $area_familiar = $data->area_familiar;

            $familiares_empresa = array();

            for($i=0;$i<count($nombre_familiar);$i++){

                if($nombre_familiar[$i]!='' && $apellido_familiar[$i]!='' && $cargo_familiar[$i]!='' && $area_familiar[$i]!=''){

                    $familiar=array(
                        "nombre"=>$nombre_familiar[$i],
                        "apellido"=>$apellido_familiar[$i],
                        "cargo"=>$cargo_familiar[$i],
                        "area"=>$area_familiar[$i]);

                    array_push($familiares_empresa,$familiar);
                }
            }

            $nueva_entrevista->familiares=json_encode($familiares_empresa);
            ...
            for($i=0;$i<count($titulo);$i++){
                ...
                if($titulo[$i]!='' && $institucion[$i]!='' && $ano[$i]!='' && $ciudad[$i]!=''){
                    $est=array("nombre"=>$titulo[$i],"edad"=>$institucion[$i],"ocupacion"=>$ano[$i],"ciudad"=>$ciudad[$i]);

                    array_push($estudio,$est);
                }
            }

            $nueva_entrevista->titulos=json_encode($estudio);


            if($data->empresa!=null){
                $empresa=$data->empresa;    
                $cargo_desem=$data->cargo_desem;
                ...
                for($i=0;$i<count($empresa);$i++){
                    ...
                    if($empresa[$i]!='' && $cargo_desem[$i]){
                        
                        $motivo_retiro_desc = MotivoRetiro::where("id", $motivo_retiro[$i])->pluck("descripcion")->first();

                        $exp=array("empresa"=>$empresa[$i],"cargo"=>$cargo_desem[$i],"jefe_inmediato"=>$jefe_inmediato[$i],"salario"=>$salario[$i],"horario"=>$horario[$i],"ingreso"=>$ingreso[$i],"retiro"=>$retiro[$i],"motivo_retiro"=>$motivo_retiro_desc,"funciones"=>$funciones[$i],"beneficios"=>$beneficios[$i]);

                        array_push($experiencias,$exp);
                    }
                }

                $nueva_entrevista->experiencias=json_encode($experiencias);

            }

        
        $nueva_entrevista->save(); //guardao luego de llenar los campos json
    ...
    public function registra_proceso_entidad(Request $data)
    {
       ...
       //$ae = GestionPrueba::find($data->prueba_id);
        ...
        $proceso_req=ProcesoRequerimiento::where("entidad_id",$data->get("entrevista_id"))
            ->where("tipo_entidad","MODULO_ENTREVISTA_SEMIESTRUCTURDA")
    ...
    public function generar_pdf($req_can){
      ...
      $reqcandidato = ReqCandidato::join("requerimientos", "requerimientos.id", "=", "requerimiento_cantidato.requerimiento_id")
        ...
        $view = \View::make('admin.reclutamiento.entrevista_semi', compact(
            'entrevistas_semi','reqcandidato'
        ))->render();

        $pdf = \App::make('dompdf.wrapper');
        $pdf->loadHTML($view);

       return $pdf->stream('entrevista_semiestructurada.pdf');
