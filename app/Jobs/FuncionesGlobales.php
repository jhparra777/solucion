...
{   /*constantes que representan la categoria  DOCUMENTOS BENEFICIARIOS*/
    ...
    //puntuacion de evaluacion sst
    public static function puntuacion_sst($resp)
    {
      $suma = 0; //cuanto porcentaje
      //foreach($resp as $item){
        $suma += ($resp->preg_uno == "c")?12.5:0;
        //una
        $suma += ($resp->preg_dos    == "a")?12.5:0;
        $suma += ($resp->preg_tres   == "b")?12.5:0;
        $suma += ($resp->preg_cuatro == "e")?12.5:0;
        $suma += ($resp->preg_cinco  == "a")?12.5:0;
        $suma += ($resp->preg_seis   == "a")?12.5:0;
        $suma += ($resp->preg_siete  == "d")?12.5:0;
        $suma += ($resp->preg_ocho   == "b")?12.5:0;
      //}
            ...
            case "link":
                if ($usuario->hasAccess($permiso)) {
                    ...
                    if (isset($opciones['id'])) {
                        $url = route($permiso, ['id' => $opciones['id']]);
                    } else {
                        $url = route($permiso);
                    }
    ...
    */
    ...
    public static function tieneCitaAsignada($requerimiento_candidato_id)
    {
        if ( is_null($requerimiento_candidato_id) ) {
            return false;
        }

        $cita = Citacion::where("req_candi_id", $requerimiento_candidato_id)->first();

        return isset($cita) ? true : false;
    }
